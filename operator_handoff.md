...
- **Scope:** Close the four panel/gate divergences that commit `8321caf2` documented as "known divergence" when it cherry-picked the alignment test without the code: (1) wire `isGroundedEvidenceInActiveFiles` (quote containment + page <= totalPages) into the canonical resolver, Metadata Truth panel, generation gate, release snapshot, final-submission readiness, BuildPlan hash, legacy reconciliation, and the generate/metadata-override routes — it was dead code with zero production callers; (2) resolver derives the EFFECTIVE (override-aware) submission method for conditional criticality and the email-subject rule, matching the validator's `effectiveValue()`; (3) `submissionEmails` conditionally critical on email methods (registry) + portal one-of-two grounded-endpoint post-pass in the resolver, mirroring the validator's endpoint branch exactly (email-subject rule follows the chosen portal endpoint); (4) restored the alignment test's strict panel==gate assertions that `8321caf2` had relabelled "known divergence". Also fixed `isEmailSubmissionMethod`: `\bemail\b` never matched enum forms (`EMAIL_SUBMISSION`) because `_` is a word character — physical/portal classifiers already handled underscores, email did not, so enum-form email tenders false-blocked as unclassifiable.
- **Files changed:** `lib/engine/canonical-field-state.ts`, `lib/engine/tender-policy-registry.ts`, `lib/engine/submission-method-policy.ts`, `lib/engine/analysis/metadata-truth.ts`, `lib/engine/build-plan.ts`, `lib/engine/build-plan-hash.ts`, `lib/engine/generation-readiness-gate.ts`, `lib/engine/tender-release-snapshot.ts`, `lib/engine/final-submission-readiness.ts`, `lib/engine/legacy-tender-reconciliation.ts`, generate + metadata-override routes, `tests/metadata-contradiction-alignment.test.ts` (strict assertions restored), fixture/pin updates in 5 existing test files, `operator_handoff.md`.
- **Also fixed (pre-existing failures on main, verified failing on pristine `8321caf2`):** (a) `tests/proposal-version-ownership.test.ts` (cherry-picked from PR #945 but never run against a database) failed because the proposal-versions LIST route and the `[versionId]` GET were strictly owner-scoped with no ADMIN fallback — wired the cherry-picked `requireTenderAccess` helper into both so every proposal-version route enforces the same two-tier rule; (b) `tests/db-transfer-query-shape.test.ts` + `tests/no-filecontent-in-list-endpoints.test.ts` failed because commit `f347077c`'s blob-cleanup fix legitimately selects `fileContent` inside the tender DELETE handler and the whole-file regex pins caught it — pins now scope to the GET/PUT (dashboard) portion, keeping the dashboard-payload protection intact.
- **Round 2 (same session, after PR #950 went green):** (a) workflow-center stages 5/6 showed READY from a valid-field ratio / count-based `buildPlan.valid` while the gate blocked — both now consult the gate-aligned `metadata.gateValid` / `buildPlan.gateValid` + surface `gateBlocker` (no green-but-blocked stages); (b) closed the metadata-override route's documented follow-up: a USER_EDITED value that enrichment PROVES is contained in an active file's extracted text is promoted into the tender scalar in the same update (verified manual extraction, identical to re-extract-metadata's write) so the resolver and BuildPlan validator both see it grounded; unevidenced values never touch the scalar (fail closed). Regression pins added to tests/panel-unification.test.ts and tests/metadata-override-source-enrichment.test.ts.
- **Round 3 (same session):** fixed the dead `extractionTruncated` flag — the 500K inner limiter in `lib/extract-text.ts` fired before the 2M outer check in `limitExtractedText`, so the flag was documented as "effectively always false" and a document over 500K chars was silently partial (CLAUDE.md requires a partial-extraction warning). `limitExtractedText` now detects text cut at the exported inner cap and reports `truncated=true`; both upload paths already surface the flag as a user-visible warning. Behavioral tests added (at-cap → truncated, below-cap → not, over-2M → sliced+truncated); the stale round-9 "documented conflict" pin updated to pin the fix instead.
- **Round 4 (same session):** completed round 7's deferred cleanup — deleted the orphaned duplicate resolver `lib/engine/analysis/metadata-truth.ts` (ZERO runtime callers: the Metadata Truth panel reads the workflow-center snapshot, which uses `resolveCanonicalFieldState`; a second resolver was the original source of the two-thresholds contradiction and a standing drift risk). The four referencing test files were retargeted, not weakened: the type import now comes from `canonical-field-state` (which re-exports `MetadataFactStatus`), the "both resolvers" consistency test became a keep-it-deleted pin, and the obsolete source-inspection section is documented as superseded by the canonical-resolver pins.
- **Tests run:** `npx tsc --noEmit` PASS; `npm run lint` PASS; `npx prisma validate` + `migrate deploy` on fresh PostgreSQL 16 PASS; `RUN_DB_INTEGRATION=true` full suite PASS (see PR for exact count); `npm run build` PASS.
- **Known risks:** Panels are now exactly as strict as `validateCriticalMetadataEvidenceForBuildPlan` — tenders whose evidence quotes are not contained in the active file's extracted text (or whose source page exceeds totalPages) stop showing green and require re-extract/repair, which is the intended fail-closed behavior. `metadata-override` and final-submission selects now load `extractedText` for active files (same as the gate/snapshot already did).
- **Next action:** Hope reviews the PR; do not merge or deploy without approval.
- **Merge status:** `unsafe` — all local checks pass; awaiting CI and Hope's review.
