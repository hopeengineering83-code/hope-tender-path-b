...
- **Gate on the committed tree:** `prisma validate` valid; `tsc` clean; `next lint` clean; `git diff --check` clean; `npm test` with `RUN_DB_INTEGRATION=true` **12595 / 12595 pass** on `7a8fac36`, **12599 / 12599 pass, 0 fail, 0 skipped** with the dated-reference fix; `next build` exit 0 both times. Local full pipeline twice: AUTO_FINALIZE ok, ZIP HTTP 200, 32 pages, geometry check clean.
