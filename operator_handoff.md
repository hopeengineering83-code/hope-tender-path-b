...
### 2026-07-18 UTC (follow-up 28) — Codex

- **Mode:** continued SG-023 direct asset-byte privacy. Found Company asset download responses still used the original uploaded filename in `Content-Disposition`, unlike the newly hardened document download path.
...
### 2026-07-18 UTC (follow-up 27) — Codex

- **Mode:** continued SG-023/SG-028 direct-byte and activity privacy. Found Company document download responses used the original uploaded filename in `Content-Disposition`, and document re-extraction audit descriptions included the raw uploaded filename.
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata to be refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `app/api/company/documents/[id]/route.ts` now uses opaque `company-document-<id>` download filenames while preserving extension and changes the public audit description to `Re-extracted company document`; raw original filename remains only in internal metadata; `tests/company-documents-privacy-dto.test.ts` locks the contract; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/tender-currency-no-default-current.test.ts tests/currency-authority-current.test.ts tests/matching-fail-closed-selection.test.ts tests/company-batch-review-rbac-current.test.ts tests/company-documents-privacy-dto.test.ts tests/activity-safe-presentation.test.ts tests/password-reset-token-policy.test.ts tests/password-reset-safe-sql-current.test.ts tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (61/61); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; no merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** This closes a direct document filename/activity-description privacy subgap only; full SG-023/SG-028 still requires browser and stock PostgreSQL proof.
- **Next action:** run focused and repository checks, then refresh the draft PR metadata.
- **Merge status:** not reviewed; draft-only; not safe to merge as final closure.

### 2026-07-18 UTC (follow-up 26) — Codex

- **Mode:** continued SG-023 client-side bounded DTO consumption. `/api/company/documents` already had cursor pagination/default limits, but the Company dashboard still called it without an explicit limit in initial load and refresh paths.
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata to be refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `app/dashboard/company/page.tsx` now requests `/api/company/documents?limit=50` for both initial document loading and refresh; `tests/company-documents-privacy-dto.test.ts` locks the bounded document client request; matrix and handoff updated.
...
- **CI/deployment status:** not checked; no merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** This closes an explicit client request subgap only; full SG-023 still requires browser progressive-disclosure screenshots and stock PostgreSQL pagination/tenant tests.
- **Next action:** run focused and repository checks, then refresh the draft PR metadata.
- **Merge status:** not reviewed; draft-only; not safe to merge as final closure.
...
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata to be refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `app/api/company/compliance-records/route.ts` and `app/api/company/legal-records/route.ts` now parse bounded `page`/`limit`, use `take`/`skip`, count total rows, add `id` deterministic ordering tie-breakers, and return pagination metadata; `app/dashboard/company/page.tsx` requests `?limit=50`; `tests/company-documents-privacy-dto.test.ts` locks the compliance/legal pagination contract; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/tender-currency-no-default-current.test.ts tests/currency-authority-current.test.ts tests/matching-fail-closed-selection.test.ts tests/company-batch-review-rbac-current.test.ts tests/company-documents-privacy-dto.test.ts tests/activity-safe-presentation.test.ts tests/password-reset-token-policy.test.ts tests/password-reset-safe-sql-current.test.ts tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (59/59); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; no merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** This closes compliance/legal list pagination subgaps only; full SG-023 still requires browser progressive-disclosure screenshots and stock PostgreSQL pagination/tenant tests.
- **Next action:** run focused and repository checks, then refresh the draft PR metadata.
- **Merge status:** not reviewed; draft-only; not safe to merge as final closure.

### 2026-07-18 UTC (follow-up 23) — Codex

- **Mode:** continued SG-023 privacy/progressive-disclosure on the financial evidence list. Found `/api/company/financial-records` still returned the full same-company collection without server pagination or an id tie-breaker, even after role minimization.
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata to be refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `app/api/company/financial-records/route.ts` now parses bounded `page`/`limit`, uses `take`/`skip`, counts total rows, adds `id` as a deterministic ordering tie-breaker, and returns pagination metadata; `app/dashboard/company/page.tsx` requests `?limit=50`; `tests/company-documents-privacy-dto.test.ts` locks the pagination contract; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/tender-currency-no-default-current.test.ts tests/currency-authority-current.test.ts tests/matching-fail-closed-selection.test.ts tests/company-batch-review-rbac-current.test.ts tests/company-documents-privacy-dto.test.ts tests/activity-safe-presentation.test.ts tests/password-reset-token-policy.test.ts tests/password-reset-safe-sql-current.test.ts tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (58/58); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; no merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** This closes a financial-record list pagination subgap only; full SG-023 still requires browser progressive-disclosure screenshots and stock PostgreSQL pagination/tenant tests.
- **Next action:** run focused and repository checks, then refresh the draft PR metadata.
- **Merge status:** not reviewed; draft-only; not safe to merge as final closure.

### 2026-07-18 UTC (follow-up 22) — Codex

- **Mode:** continued SG-021/SG-023 privacy/RBAC minimization. Found `/api/company/financial-records` allowed `REVIEWER` role to read amount/currency financial evidence rows, while mutation was already manager-only and reviewers should not receive unnecessary financial details by default.
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata to be refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `app/api/company/financial-records/route.ts` now restricts financial record list reads to `ADMIN` and `PROPOSAL_MANAGER`; `tests/company-documents-privacy-dto.test.ts` locks the manager-only financial list contract; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/tender-currency-no-default-current.test.ts tests/currency-authority-current.test.ts tests/matching-fail-closed-selection.test.ts tests/company-batch-review-rbac-current.test.ts tests/company-documents-privacy-dto.test.ts tests/activity-safe-presentation.test.ts tests/password-reset-token-policy.test.ts tests/password-reset-safe-sql-current.test.ts tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (57/57); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; no merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** This narrows one API surface but is still not full SG-021/SG-023 closure without stock PostgreSQL role/isolation tests and browser proof.
- **Next action:** run focused and repository checks, then refresh the draft PR metadata.
- **Merge status:** not reviewed; draft-only; not safe to merge as final closure.

### 2026-07-18 UTC (follow-up 21) — Codex

- **Mode:** continued SG-026 currency truth in company evidence entry. Found Company project forms, edit forms, financial record form, and the financial-records API still defaulted blank currency to `USD`, which could turn unresolved company evidence currency into a false authoritative currency.
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata to be refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `app/dashboard/company/page.tsx` now initializes/resets project and financial currency controls blank, exposes `Currency unresolved` options, and preserves blank project edit currency; `app/api/company/financial-records/route.ts` now writes null for omitted currency; `tests/tender-currency-no-default-current.test.ts` locks the company evidence no-USD-default contract; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/tender-currency-no-default-current.test.ts tests/currency-authority-current.test.ts tests/matching-fail-closed-selection.test.ts tests/company-batch-review-rbac-current.test.ts tests/company-documents-privacy-dto.test.ts tests/activity-safe-presentation.test.ts tests/password-reset-token-policy.test.ts tests/password-reset-safe-sql-current.test.ts tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (56/56); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; no merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** This closes another SG-026 entry/API defaulting subgap only; schema still has legacy defaults for unrelated/prior-state compatibility and full missing/conflicting/stale currency PostgreSQL + browser/export proof remains required.
- **Next action:** run focused and repository checks, then refresh the draft PR metadata.
- **Merge status:** not reviewed; draft-only; not safe to merge as final closure.

### 2026-07-18 UTC (follow-up 20) — Codex

- **Mode:** continued SG-026 currency truth in deterministic matching. Found `lib/engine/matching.ts` still defaulted missing project contract-value currency to `USD` in match rationale text.
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata to be refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `lib/engine/matching.ts` now emits `Currency unresolved` for project contract-value rationale when currency is missing; `tests/tender-currency-no-default-current.test.ts` locks the matcher no-USD-default contract; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/tender-currency-no-default-current.test.ts tests/currency-authority-current.test.ts tests/matching-fail-closed-selection.test.ts tests/company-batch-review-rbac-current.test.ts tests/company-documents-privacy-dto.test.ts tests/activity-safe-presentation.test.ts tests/password-reset-token-policy.test.ts tests/password-reset-safe-sql-current.test.ts tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (55/55); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; no merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** This closes one deterministic-matcher rationale defaulting subgap only; full SG-026 still requires current extraction authority plus stale/missing/conflicting currency PostgreSQL and browser/export proof.
...
### 2026-07-18 UTC (follow-up 19) — Codex

- **Mode:** continued SG-026 currency truth beyond prompt context. Found manual tender creation (`app/api/tenders/route.ts`) and upload-first tender creation (`lib/tender-upload-first.ts`) still persisted `USD` when request/extraction currency was missing.
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata to be refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `app/api/tenders/route.ts` now writes `currency: body.currency || null`; `lib/tender-upload-first.ts` now writes `currency: metadata.currency ?? null`; `tests/tender-currency-no-default-current.test.ts` locks manual/upload-first null behavior and report unresolved display assumptions; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/tender-currency-no-default-current.test.ts tests/currency-authority-current.test.ts tests/matching-fail-closed-selection.test.ts tests/company-batch-review-rbac-current.test.ts tests/company-documents-privacy-dto.test.ts tests/activity-safe-presentation.test.ts tests/password-reset-token-policy.test.ts tests/password-reset-safe-sql-current.test.ts tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (54/54); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; no merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** This closes two intake-defaulting subgaps only; full SG-026 still requires current extraction authority plus stale/missing/conflicting currency PostgreSQL and browser/export proof.
- **Next action:** run focused and repository checks, then continue another non-overlapping matrix row with implementation plus tests.
- **Merge status:** not reviewed; draft-only; not safe to merge as final closure.

### 2026-07-18 UTC (follow-up 18) — Codex

- **Mode:** continued SG-026 currency truth and SG-024 prompt alignment. Found `lib/engine/ai-multi-perspective-matcher.ts` still defaulted missing project rematch currency to `USD` in prompt context and instructed that weak best-available candidates/references could be selected.
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata to be refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `lib/engine/ai-multi-perspective-matcher.ts` now emits `Currency unresolved` when project currency is missing despite a contract value, and the matcher system prompts instruct weak/below-threshold candidates and references to remain unselected; `tests/currency-authority-current.test.ts` locks the currency/no-weak-best-available prompt contract; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/currency-authority-current.test.ts tests/matching-fail-closed-selection.test.ts tests/company-batch-review-rbac-current.test.ts tests/company-documents-privacy-dto.test.ts tests/activity-safe-presentation.test.ts tests/password-reset-token-policy.test.ts tests/password-reset-safe-sql-current.test.ts tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (51/51); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; no merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** This closes a prompt-context currency/defaulting subgap only; full SG-026 still requires field-specific current extraction authority plus stale/missing/conflicting currency PostgreSQL and browser/export proof.
- **Next action:** run focused and repository checks, then continue another non-overlapping matrix row with implementation plus tests.
- **Merge status:** not reviewed; draft-only; not safe to merge as final closure.

### 2026-07-18 UTC (follow-up 17) — Codex

- **Mode:** continued SG-024 by hardening AI rematch against bypassing the deterministic fail-closed matcher. Found `/api/tenders/[id]/ai-rematch` still applied top-N existing engine matches when provider scoring was unavailable and selected AI assessments without a server-side score/critical-floor eligibility filter.
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata to be refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `app/api/tenders/[id]/ai-rematch/route.ts` now returns `AI_REMATCH_UNAVAILABLE_NO_SELECTION` without applying fallback selections when provider scoring is unavailable, and filters AI selections through `overallScore >= 0.75`, critical floor >= 5, and explicit `recommendSelection`; `tests/matching-fail-closed-selection.test.ts` locks this route contract; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/matching-fail-closed-selection.test.ts tests/company-batch-review-rbac-current.test.ts tests/company-documents-privacy-dto.test.ts tests/activity-safe-presentation.test.ts tests/password-reset-token-policy.test.ts tests/password-reset-safe-sql-current.test.ts tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (49/49); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; no merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** This closes another SG-024 direct route subgap, but persistence invalidation, reviewed-evidence revision binding, cross-company matching, browser workflow, and PostgreSQL negative tests remain required before full SG-024 closure.
- **Next action:** run focused and repository checks, then continue another non-overlapping matrix row with implementation plus tests.
- **Merge status:** not reviewed; draft-only; not safe to merge as final closure.
...
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `app/api/company/documents/route.ts`, `app/api/company/experts/route.ts`, and `app/api/company/projects/route.ts` now use shared local `parseBoundedLimit()` helpers with default 50/max 100 behavior and deterministic id tie-breaker ordering; `tests/company-documents-privacy-dto.test.ts` locks fail-safe pagination and ordering for all three list DTO endpoints; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/company-documents-privacy-dto.test.ts tests/activity-safe-presentation.test.ts tests/password-reset-token-policy.test.ts tests/password-reset-safe-sql-current.test.ts tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (40/40); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; GitHub CLI/direct fetch unavailable. No merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** SG-023 list DTO pagination is safer, but PostgreSQL proof, progressive-disclosure/browser screenshots, and any remaining large-page UX defects still need execution before full closure.
...
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `app/dashboard/company/review/page.tsx` now fetches `/api/company/review-summary`, `/api/company/documents?limit=50`, `/api/company/experts?limit=50`, and `/api/company/projects?limit=50` instead of the full `/api/company` graph; it uses summary counts for totals and bounded list DTO items for rendered records. `tests/company-documents-privacy-dto.test.ts` now locks this bounded-loading contract; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/company-documents-privacy-dto.test.ts tests/activity-safe-presentation.test.ts tests/password-reset-token-policy.test.ts tests/password-reset-safe-sql-current.test.ts tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (38/38); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; GitHub CLI/direct fetch unavailable. No merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** Company Review no longer fetches the full graph, but SG-023 still needs deeper expert/project presentation minimization, stock PostgreSQL proof, and browser screenshot proof before full closure.
- **Next action:** run repository checks for this follow-up, then continue SG-023 progressive-disclosure/minimized DTOs or another non-overlapping matrix row with implementation plus tests.
- **Merge status:** not reviewed; draft-only; not safe to merge as final closure.
...
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `lib/secure-password-reset.ts` now exports `classifyPasswordResetToken()` / `ResetTokenState`, uses that policy inside the transactional reset flow, and marks expired unconsumed rows consumed before returning the sanitized invalid-reset response; `tests/password-reset-token-policy.test.ts` adds behavior coverage for active/missing/consumed/expired classification plus expired-token revocation and same-transaction one-time semantics; `tests/password-reset-safe-sql-current.test.ts` now locks the centralized classifier contract; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/password-reset-token-policy.test.ts tests/password-reset-safe-sql-current.test.ts tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/activity-safe-presentation.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (32/32); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; GitHub CLI/direct fetch unavailable. No merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** reset-token expiry/one-time/revocation semantics are now implemented and locally tested, but SG-030 still needs stock PostgreSQL execution, durable private storage/recovery implementation proof, and screenshots before full closure.
- **Next action:** run repository checks for this follow-up, then continue the remaining SG-030 storage/recovery subgap or another non-overlapping matrix row only with implementation plus tests.
- **Merge status:** not reviewed; draft-only; not safe to merge as final closure.

### 2026-07-18 UTC (follow-up 6) — Codex

- **Mode:** continued SG-030 by fixing the client flow after inspecting `/forgot-password` and `/reset-password`. Found two real contradictions: forgot-password still claimed it would generate a reset link and had code to render/copy raw `resetLink` fields if present; reset-password required a `uid` query param even though the server-generated reset URL only contains `token`.
- **Branch / PR:** `fix/final-screenshot-root-gap-closure` / draft PR metadata refreshed for `fix: close remaining screenshot root gaps and release blockers`; requested base remains `release/consolidated-recovery-20260717`. Live GitHub fetch remains unavailable (`gh` absent, no configured remote, direct GitHub fetch 403).
- **Exact scope and files changed:** `app/forgot-password/page.tsx` now requests reset instructions instead of claiming to generate a link and no longer renders/copies raw reset-link fields; `app/reset-password/page.tsx` now accepts the token-only link generated by `forgot-password` and no longer requires/sends `uid`; `tests/password-reset-client-contract.test.ts` locks those client/server contracts; matrix and handoff updated.
- **Tests/checks actually run:** `npx tsx --test tests/password-reset-client-contract.test.ts tests/password-reset-mail-configuration.test.ts tests/activity-safe-presentation.test.ts tests/final-screenshot-gap-closure-matrix.test.ts` PASS (24/24); `git diff --check` PASS; `npm run typecheck -- --pretty false` PASS; `npm run lint` PASS; `DATABASE_URL=postgresql://test:test@localhost:5432/test npx prisma validate` PASS; `npm run audit:release-integrity` PASS.
- **CI/deployment status:** not checked; GitHub CLI/direct fetch unavailable. No merge, deployment, preview, production data access, or production migration.
- **Known risks/assumptions:** SG-030 reset-mail/client flow is significantly stronger, but full closure still needs reset-token PostgreSQL behavior, durable storage/recovery docs, and screenshots.
- **Next action:** continue one remaining matrix subgap at a time; for SG-030, next best proof is PostgreSQL behavior for reset token expiry/one-time use/revocation if a real DB becomes available.
- **Merge status:** not reviewed; draft-only; not safe to merge as final closure.
