...
- **Tests so far:** the pre-edit Phase 1 suite passed 16/16; the exact old Stage 3 source path was reproduced returning the unconditional automatic-continuation sentence; 53 focused Phase 1/Phase 2/manual-button tests pass; 28 additional workflow-center/two-action/route semantic tests pass; typecheck passes; lint passes with the one standing unused-disable warning. On first pushed head `a1846ef`, one CI full suite reached 9,868/9,869 with the standing nondeterministic owner-workflow PDF validation failure while the other passed the full real-PostgreSQL unit phase and reached Playwright. On `b9cf39b`, `72b432f`, and `c2c557c`, both full real-PostgreSQL unit phases passed. The browser fixture exposed three setup-only issues before the target screen: nested Build Plan transaction behavior, Playwright's CommonJS transform of a server import, then the production `Secure` session cookie correctly not being sent over loopback HTTP. It now follows the established E2E auth helper contract: authenticate through the real API, verify `Set-Cookie`, and inject only the test copy with `secure:false`; production cookie policy remains unchanged. The fixture calls the real authenticated `/build-plan` and `/engine-readiness` endpoints, then persists only the successful current-revision Engine fact. Exact-head CI must verify this correction. Local PostgreSQL remains unavailable because the configured Neon host is unreachable and this container has no local server.
