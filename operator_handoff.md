...
- **Owner re-upload + authorized run (tender `75552666-…`):** vault 6/6 extracted, 100 projects, 28 experts, 3 brand assets verified; tender 7/7 pages. Owner authorized AI Analyze + Run Engine. Acceptance run 36256578677: AI Analyze SUCCEEDED; ENGINE_RUN FAILED `BUILD_PLAN_AUTOMATION_BLOCKED` (ref cef6eba9): "Build Plan item count does not match current tender-controlled submission scope. Technical Proposal.pdf order/type/name is not in current tender-controlled scope." **Root cause (reproduced from the real rows, printed by the new inspection step, run 36257221959):** the one file requirement is exactFileName `Technical Proposal.pdf` with restrictions "PDF electronic submission only. Financial proposal is excluded." The draft preflight loaded requirements without `restrictions` and planned the file; the confirmation re-derivation loaded full rows, the restrictions became the file's notes, and the classifier read "Financial proposal is excluded" as a no-financial rule and dropped the file (plan `[]`). **Fix:** (1) `classifySubmissionPlanItem`: a row whose exact file name positively names a non-financial bidder deliverable is that deliverable whatever its description/restrictions say (names that are themselves financial/separation statements keep the old checks; `plannedFileIsARule` inherits this); (2) the draft preflight now selects every requirement field `buildSubmissionPlan` reads (`requiredQuantity`, `pageLimit`, `restrictions`, `sectionReference`), so draft and confirmation plan from identical inputs. Test: `tests/a-named-deliverable-is-not-a-rule-because-of-its-restrictions.test.ts`. Gate: `tsc`/`lint`/`diff --check` clean; `npm test` with `RUN_DB_INTEGRATION=true` **12606 / 12606 pass, 0 skipped**; `next build` exit 0.
...
- **Hosted acceptance of `7a8fac36` (run 36243835992):** generation SUCCEEDED; AUTO_FINALIZE failed `AUTO_FINALIZE_NOT_CONVERGED` / `GENERATED_DOCUMENT_QUALITY_FAILED` (ref c1bf2c51). Vercel runtime log: `Technical Proposal.pdf` score 85, `issueCodes: ["PRICING_LEAKAGE"]`. The read-only `confirm=inspect` run 36244490458 named the fragment: `currency amount in "| ETB 550.1M; 2015-2018; Services:"`. The model-written section had copied a reference-project record (`projectReferenceLine`: "Construction value of works ETB 550.1M | 2015-2018 | Services: …") into a table cell without its label, and the PDF wrapped the cell so the amount reached the gate beside only its dates. **Fix (pricing-hygiene.ts):** `namesClosedPastPeriod` — an amount dated to a year range that ended before the current year is delivered work, not this bid's price; consulted only after the current-offer veto, so "our fee … 2015-2018", "Total price … 2015-2018" and bare "Construction supervision: <amount>" are still caught. Test: `tests/a-reference-value-dated-to-a-closed-period-is-not-a-price.test.ts` (the exact hosted line reproduced failing before the change).
- **Hosted acceptance of `0abeb82f` (run 36246053472, Preview `dpl_BJd14XCZStD7QmGK29DB2oPFnsco`):** AI Analyze → Run Engine → generation → AUTO_FINALIZE all SUCCEEDED; export-readiness `READY`, 0 blockers; audit `Technical Proposal.pdf` score **100** PASSED; ZIP HTTP 200; 31 pages, geometry check clean (30/31 labelled, cover unnumbered by design). The delivered Cover Letter and Executive Summary are the record-based ones; every registration prints as "<designation>, Reg. No. <number>" with no nested brackets. **One more defect in that artifact, fixed at source:** "led byAhmed …", "andHabib …", "supported byElias …" — `enforceCanonicalNames` (entity-name-normalizer.ts, AI path only) matched `\b(?:title)?\s*First … Last`; with no title the `\s*` swallowed the space after a preceding word and the canonical replacement dropped it. The space now belongs to the optional title. Test: `tests/canonical-names-keep-the-space-before-them.test.ts` (fails on the old pattern).
...
- **Gate on the committed tree:** `prisma validate` valid; `tsc` clean; `next lint` clean; `git diff --check` clean; `npm test` with `RUN_DB_INTEGRATION=true` **12595 / 12595 pass** on `7a8fac36`, **12599 / 12599** with the dated-reference fix, **12602 / 12602 pass, 0 fail, 0 skipped** with the name-spacing fix; `next build` exit 0 each time. Local full pipeline twice: AUTO_FINALIZE ok, ZIP HTTP 200, 32 pages, geometry check clean.
- **Merge status:** **DO NOT MERGE**.
...
- **Codex work verified and preserved:** `bindRequirementEvidenceToActiveFile` (quote-to-active-file binding, canonical page resolution, ambiguous/foreign/missing evidence still blocked) is untouched; its regression suite passes in the full run. The navy header / white text / zebra rows / gold cover accent / anchored table top border are kept.
- **How the defects were found:** the fallback Pharo proposal was generated through the real pipeline locally (upload → extraction → AI Analyze → Run Engine → generation → AUTO_FINALIZE → ZIP), the ZIP's `Technical Proposal.pdf` rasterised, and **every page** inspected; each defect was traced to its builder before changing anything.
- **Root causes and fixes (all generic, none Pharo-specific):**
  1. *Every heading rendered as body text, no section page breaks, bullets without bullets.* The PDF is drawn from markdown re-extracted out of the DOCX, and `extractDocxMarkdownText` read runs only, never paragraph styles. `export-readiness.ts` now maps `Heading1-6` → `#…`, list numbering → `- `, the contents title → `# Table of Contents`, and separates the DOCX cover (`extractDocxProposalParts`). Style ids are shared in `docx-paragraph-styles.ts`.
  2. *Every table's header band was empty, in the DOCX and the PDF.* `parseMdTable` built header cells as `new TextRun({ ...run, bold: true })`; spreading a TextRun copies no text. Bold is now a `parseInlineRuns` option; header rows also repeat on each page (`tableHeader`).
  3. *Contents listed twice in the DOCX; in the PDF a plain list with no page numbers.* `markdownToDocx` ended the static-TOC skip at the blank line directly under the heading. The PDF now builds the contents from the document's own H1/H2 headings with dot leaders and the real page numbers (filled in after layout).
  4. *The DOCX cover block printed again at the top of page 2.* Cover paragraphs carry `ProposalCover` / `ProposalCoverDetail` styles; the body extractor skips them and the record facts (TIN/VAT/grade, signatory, date/validity, service lines) go to the PDF cover. The DOCX cover now ends with a page break.
  5. *Sections E and F were subsections of D.* Their builders emitted `##`; they now emit `#` like every other lettered section (the last-resort repair addenda stay at `##`, see note below).
  6. *Pagination:* each top-level section opens a page; headings keep with their lead-in and with the table they introduce (the whole table when it is short enough to be kept whole); short tables are never split; a long table never carries one row alone; paragraphs have widow/orphan control; list markers move with their first line.
  7. *Tables:* column widths from content (auto-layout, minimum = longest word) instead of equal widths; no extra blank line per row.
  8. *Typography:* body 10.5 pt / 15 pt leading; H1 18 pt navy with a gold rule; H2 13.5 pt navy; H3 11.5 pt slate; table 9 pt; numbered items keep their numbers; nested bullets indent.
  9. *Cover:* taller navy band sized to the title, gold "TECHNICAL PROPOSAL" label, labelled Submitted to / Prepared by blocks; no running header or page number on the cover (it still counts as page 1 so the contents numbers match the footer).
  10. *Declaration:* the fallback declaration now carries the Signature / Stamp / Date rule, drawn in the PDF as three labelled signing lines (the vault's signature/stamp images are still drawn above it only when they exist; nothing is fabricated).
  11. *"Pharo Ventures's":* one `possessive()` helper at the client-/company-name sites.
- **Content batch in the same commit (from the earlier 76→90 work):** team ordered as the firm is organised; Executive Summary composed from records (`executive-summary-composer.ts`); A.1 corporate facts from the profile; portfolio relevance = the tender scope items the project's recorded services answer; relative work-plan sequencing when the tender states no duration; the internal tail blocks removed from the fallback; the service-stream methodology block no longer duplicates Section C (wiring test updated to the new contract).
- **Note for the next agent:** do not change `proposal-quality-repair.ts` Section E/F headings to `#`. Its copies arrive inside the internal matrix block, which is stripped as a unit; at `#` they escaped the strip and replaced the builders' better Sections E/F (observed and reverted).
- **Local full-proposal result (fallback path, `BENCH_DRAFT_MODE=unavailable`):** 32 pages, AUTO_FINALIZE ok, export READY, ZIP HTTP 200; all 32 pages rendered and inspected; no clipping, overlap, orphaned heading, stranded row or duplicate cover/TOC.
- **Tests (run on the committed tree):** `prisma validate` valid; `prisma generate` ok; `tsc --noEmit` clean; `next lint` no warnings or errors; `npm test` with `RUN_DB_INTEGRATION=true` on local PostgreSQL **12573 tests, 12573 pass, 0 fail, 0 skipped**; `next build` exit 0; `git diff --check` clean. Updated to the new contracts: `proposal-pdf-renders-unicode` (the cover is unnumbered but counted, so footers run 2..N of N), `generated-document-quality-final-enforcement` #6 (no second service-stream methodology block), `tender-workflow-e2e-gates` (finalizer uses `extractDocxProposalParts`). New: `tests/proposal-pdf-table-layout.test.ts` (rewritten to behaviour: DOCX headers, one TOC, cover page break, extractor structure, section pages + TOC page numbers, numbered lists, short-table keep-together, column widths, TOC planning) and `tests/a-fallback-proposal-reads-as-the-firm-is-organised.test.ts`.
- **Next action:** exact-head CI + Preview, then a fresh hosted Pharo generation, download, render every page, iterate.
...
**Further commits this session (all generic, each with a regression test):**

| Commit | Defect seen on Preview | Root cause | Fix |
|---|---|---|---|
| `83f659d0` | AUTO_FINALIZE UNSUPPORTED_CLAIM_RISK on the first model-written section (run 36047880422) | Writer prompts asked for "we have already delivered this assignment" / "the same team" | Prompts ask for the closest comparable project; claim patterns shared by the gate and `clientSafeModelSection` |
| `7e8f211a` | Run 36049851073 passed at quality 95 yet delivered 22 pages of the writer's contract / criterion graph, "Pharo Ventures requires Pharo Ventures", "founded 2012 / Grade A / ISO 45001" (record: 2019, Grade I, no such ISO) | Partial-fallback path never exercised | Internal appendix stripped by name + INTERNAL_TRACEABILITY gate; contract block removed before the section fallback reads input; tender title protected from the client-name enforcer; model credentials must be in the company data; CV-grounded team table, bios, licences; per-currency tiles |
| `a88fb641` | WEAKPROOFSIGNAL (run 36055406065) | Evidence-graph text reached the body | Engine identifiers in the gate list |
| `832eb295` | Turnover sentence refused as pricing (run 36059011641) | Model section stated firm's own turnover | `scrubPricingLeakageSentences` in `clientSafeModelSection` |
| `0a238c08` | Unlabelled value in why-us (run 36061396565) | Bare amount beside a project name | "construction value of works" label |
| `27303d53` | Table row "Dessie Specialized Hospital, ETB 125M, …" (run 36063806865) | The DOCX gate reads a row as cells joined with ", " | Final pricing sweep judges rows as read; blanks only the unlabelled money cell |
| `e9d3a66f` | PDF refused for "WEAKPROOFSIGNAL" and the company-profile upload header (run 36066042996) | Builders quoting vault/engine text past every section guard | `ENGINE_IDENTIFIER_PATTERNS` shared by the gate and the last pre-render sweep, with `SOURCE_DOCUMENT_METADATA_PATTERNS` |
| `96f00d5b` | DOCX passed, its PDF refused "… Hospital Project, USD 19M," (run 36068858534) | PDF wraps table cells and interleaves columns | An amount in a prose cell must carry its label in the same cell, else it is removed |
| `f50b35f9` | First gate-clean delivery (run 36071201669) still showed "aggregate value ETB 693,974,678" (USD added to ETB), the firm's past fees from the raw record ("Supervision Cost: 110,000 ETB/month"), a bullet cut to "Three-stage", a bare cover-letter amount | Why-us summed across currencies; Section B printed the writer's proof line; "internal review" is AI-trace vocabulary a repair pass cuts | Per-currency totals; `projectReferenceLine`; "design review"; labelled values; unproven "same lead" claim dropped |
| `0c6728ec` | Same delivery: Section E printed "PROPOSALRESPONSE … AUTOGENERATEDARTIFACT … Expert CVS.pdf.txt"; "VAT Registration tax compliance 15480320805"; stale "A.4 / A.4.1" cross-refs in tables; broken licensing sentence; "identified service streams: architecture, mep"; experts under the firm's discipline tags; "Weeks N–N+8" | Raw matrix fields; a VAT rewrite that ate registration numbers; the seal ignored table cells and short titles | `clientSafeComplianceEvidence` in Section E; `VAT_RATE_MENTION` spares 7+ digit numbers; seal accepts cell boundaries, trailing refs and unique opening-word titles; wording fixes |
| (this commit) | E.1 "Section D.4" for eligibility | No builder writes D.4 | Points at the Declaration |

**Failed runs and causes, in order:** 36047880422 (claim), 36055406065
(WEAKPROOFSIGNAL), 36059011641 (turnover), 36061396565 (unlabelled value),
36063806865 (table row), 36066042996 (engine text + upload header),
36068858534 (PDF-only pricing, wrapped cell). Each fixed as above.

**AFTER — run 36074770709 on `0c6728ec` (2026-09-24 23:51Z):** AI Analyze and
Run Engine (owner-authorized) → ENGINE_RUN → PROPOSAL_GENERATION →
AUTO_FINALIZE all SUCCEEDED; export-readiness READY, 0 blockers; audit
Technical Proposal.pdf 100 / PASSED; ZIP HTTP 200, 217,703 bytes, `unzip -t`
clean; 35 pages, no clipping/overflow/footer collision; ~9,300 words.
Authorship: 1 of 4 sections model-written, 3 deterministic fallback (every
other provider out of credit, invalid key or rate-limited).

Read against the tender and the authority: no engine text, no invented
credential (only "Grade 1 / Category 1" and the ISO 9001-aligned QMS
HAEC/034/23, both on record), no turnover, no fees, deadline 25 August 2026
5:00 PM correct, licences PSTE/6884 / PPA/1840 / PEPCM/5718 / PSNE/17891 as the
CVs state, 8 named experts each leading or supporting named scope items, all 6
tender scope items answered in the tender's order, values per currency and
labelled, cross-references resolve.

| # | Dimension | BEFORE `06b0cbc2` | `96f00d5b` | AFTER `0c6728ec` |
|---|---|---|---|---|
| 1 | Tender understanding | 78 | 84 | 84 |
| 2 | Evaluator alignment | 78 | 76 | 80 |
| 3 | Executive summary | 62 | 66 | 66 |
| 4 | Competitive thesis | 60 | 58 | 64 |
| 5 | Evidence density | 55 | 72 | 74 |
| 6 | Comparable-project relevance | 72 | 76 | 80 |
| 7 | Expert–role–project mapping | 35 | 78 | 78 |
| 8 | Healthcare depth | 82 | 82 | 82 |
| 9 | Methodology actionability | 70 | 80 | 80 |
| 10 | Deliverables | 75 | 74 | 76 |
| 11 | QA / QC | 80 | 80 | 80 |
| 12 | Regulatory | 70 | 72 | 74 |
| 13 | Risk | 78 | 78 | 78 |
| 14 | Tender-specific language | 84 | 86 | 86 |
| 15 | Coherence | 55 | 60 | 64 |
| 16 | Structure | 60 | 62 | 70 |
| 17 | Factual grounding | 62 | 72 | 80 |
| | **Overall** | **68** | **≈74** | **≈76** |

The first partial-fallback delivery (`83f659d0`) scored 62. The AFTER
proposal is more accurate than the benchmark on every point the benchmark
audit lists (deadline, licence numbers, unsupported authorities, "same team"),
but it is not yet as strong in narrative: the benchmark names 14 experts and 5
projects and is written as continuous prose, while three of four AFTER sections
are the deterministic draft.

**Remaining (external):** model-written sections need provider capacity —
Cerebras 402, OpenRouter credits, DeepSeek balance, Anthropic credit, Together
401 (invalid key), OpenAI 429, Gemini / Groq / Mistral / Z.ai rate limits.
**Remaining (code, small):** cover letter ends "Sincerely, <firm>" with no
signatory line; the Section G table repeats its first column; two risk
registers in C.8; `[ACTIVE]` tags in A.3.

**Tests:** `0c6728ec` full suite with `RUN_DB_INTEGRATION=true` 12,519 / 12,519;
this commit 12,520 / 12,520; `tsc --noEmit` and `next lint` clean;
`next build` clean on `f50b35f9`.
...
- **Post-PDF coverage root cause:** the first coverage reconciliation runs before PDF finalization. Finalization supersedes the DOCX row and invalidates its coverage, then validates the new PDF without recomputing coverage, leaving the email rule PARTIAL. AUTO_FINALIZE now reuses the canonical reconciliation after PDF validation and before package/final-readiness evaluation.
- **Real artifact inspection:** successful hosted run `33981128791` produced a valid one-file ZIP and a 36-page PDF with correct `Page n of 36` numbering and no out-of-bounds text blocks. Byte/text inspection still found the canonical subject concatenated with the source parser's truncated submission-address value, an invented midnight deadline in a second metadata surface, and appendix/supporting-document promises despite the one-file Build Plan. The final pre-render sweep now uses the canonical subject field, rewrites deadline text from its exact quote, and removes phantom appendix registers/references while retaining the transparent Biomedical Engineering Specialist Engagement Plan.
- **Remaining metadata root cause:** artifact `33981675726` removed internal/appendix material and retained correct whole-document numbering, but a submission-rules builder independently formatted the date-only DB field as midnight and emitted `submissionAddress` even when `submissionMethod` was Email. It now prefers the exact deadline line from tender text and omits physical portal/address output for email submissions.
- **Cover identity inspection:** rendered page 1 of artifact `33982240818` revealed the stored concatenated metadata row was accepted as the client display name, despite its grounded quote naming only Pharo Ventures. The shared client-label sanitizer now truncates at embedded procuring-entity/legal-client/project-name labels, so every downstream cover/section receives only the canonical leading entity.
- **PDF-cover root cause:** the DOCX generator used the shared cleaned client identity, but the PDF finalizer added its own cover from raw `Tender.clientName`, reintroducing the concatenated row. The finalizer now applies the same shared sanitizer at the last rendering boundary.
- **Claim-level review:** evaluator reading of the real PDF found residual same-team/sector-tenure assertions, categorical radiation/licensing and donor-standard phrasing, plus a signatory line claiming a signed copy existed. These producers now describe only reviewed records, individual professional tenure, conditional tender/authority-dependent specialist scope, and an unsigned role label; no attachment or same-team continuity is implied.
- **Final continuity sweep:** artifact `33983913314` passed hosted/package/layout gates but claim-by-claim text inspection found one remaining throughline sentence asserting the same team delivered the hospital reference. Both generic reference-throughline producers now describe only a reviewed project record and applicable lessons.
- **Byte-identity hardening:** the acceptance snapshot is taken before POST `/export`, while that call may finalize current bytes. The temporary harness now fetches the authenticated post-export audit and requires the sole ZIP entry's SHA-256 to equal the persisted current GeneratedDocument hash, in addition to ZIP structure/name checks.
- **CI correction:** the post-export hash field was added to the audit response but omitted from its Prisma projection; exact-head typecheck caught it before deployment. The projection now explicitly selects the stored hash.

### 2026-09-05 UTC — Codex (PR #1175 hosted acceptance complete; temporary cleanup)

- **Hosted proof:** exact-head run `33985276339` passed AI Analyze → Run Engine → ENGINE_RUN → PROPOSAL_GENERATION → AUTO_FINALIZE → READY → POST `/export` 200 → ZIP. The post-export SHA-256 audit matched the downloaded `Technical Proposal.pdf` bytes (`a4b445194242cd801e9df95f6bc178b6475f24d221e0ff24e3e0f412d9310d5c`) and the ZIP contained exactly that one confirmed Build Plan delivery.
- **Artifact proof:** all 37 pages rendered; page labels are sequential `Page 1 of 37` through `Page 37 of 37`; text block coordinates remain within every media box; no internal/AI/self-score/bid-review trace, placeholder, malformed portal/midnight metadata, phantom appendix, unsupported same-team/direct-equivalence claim, Moyale comparison, pricing offer, or invented named biomedical expert was found. Client, project, Addis Ababa location, verified emails, exact subject, and 25 August 2026 5:00 PM Addis Ababa Time are present.
- **Evaluator assessment:** 91/100 overall across the agreed 17 dimensions; lowest dimension 87/100. All factual/compliance gates pass. This is separate from package-integrity scoring.
- **Cleanup:** restored `.github/workflows/lockfile-refresh-artifact.yml` to the base branch's ordinary lockfile-refresh job; removed all five `scripts/tmp-*` Preview migration/acceptance helpers and the temporary admin-audit fields used only by the acceptance harness. Permanent application and regression fixes remain.
- **Next action:** require final-cleanup exact-head CI, dependency audit, route/screenshot audit, READY Preview health/release fingerprint, and clean runtime before technical merge-ready classification.
- **Merge status:** DO NOT MERGE; draft remains; Production was not modified.
- **Cleanup CI correction:** the full suite retained an old unit expectation that a generic `# Appendix Register` survives the internal-section stripper. That contradicts the now-verified one-file package rule; the regression now expects preceding client content to remain while the phantom appendix register is removed.
- **2026-09-05 Codex artifact-byte re-audit (PR #1175):** inspected the actual `Technical Proposal.pdf` from hosted run `33985276339` (SHA-256 `a4b445194242cd801e9df95f6bc178b6475f24d221e0ff24e3e0f412d9310d5c`) rather than trusting READY metadata. The 37-page artifact retained visible ellipsis-truncated running furniture and flattened-source contamination in deadline, reference, and location rows. Scope is limited to bounding submission fields, rejecting filename/table-label pseudo-facts, using exact location matches, concise non-truncating running furniture, and a fail-closed malformed-submission regression. Temporary hosted-acceptance machinery was restored solely to validate new exact-head bytes and must be removed again only after that acceptance passes. No provider ordering, database, authentication, packaging, or Production behavior was changed.
- **Hosted byte follow-up (`eeced45a`, run `33986999706`):** normal pipeline and ZIP checks passed (`READY`, zero blockers, one valid `Technical Proposal.pdf`, 36/36 page totals, no internal/AI/continuity/abattoir/truncated-email strings). Direct byte inspection nevertheless found one residual pseudo-reference (`METADATA, Title, Issuing`) and raw DOB/nationality fragments copied from CV profiles. The generic source-fact filter now rejects extraction-header tokens, and both proposed-team and detailed-bio renderers reuse the existing personal-CV-field scrubber. A second exact-head hosted acceptance is required before temporary cleanup.
- **Hosted byte follow-up (`148133e3`, run `33987454081`):** acceptance again passed with one valid 36-page PDF, correct sequential 36-page totals, exact Pharo email/deadline/method/location, and no DOB/nationality/internal/AI/overclaim strings. Actual bytes exposed a second shape of the same pseudo-reference (`Status, issued, references`). References now require identifier-like numeric content as well as rejecting extraction-table labels; this intentionally omits an absent reference instead of presenting source-table prose as one.
- **Final hosted artifact acceptance (`656a8184`, run `33987922811`):** passed end-to-end on the real Preview: `finalReadiness.ok=true`, `READY`, zero blockers, required/generated/export-ready counts 1/1/1, POST `/export` 200, and a valid ZIP containing exactly `Technical Proposal.pdf`. The downloaded PDF is 236,635 bytes (SHA-256 `298e79a01a91c22dc9ebbf4b27414b7ea215e43d51d7e090084bed78f40fe51a`), reopens, renders all 36 A4 pages, and has correct sequential `Page 1 of 36` through `Page 36 of 36`. Direct extracted-text audit found correct recipients, subject, deadline, email method, and Addis Ababa location; no pseudo-reference, DOB/nationality, AI/internal/self-score/bid-review strings, Moyale/direct-comparability/same-team claims, TODO/TBD, or visibly truncated metadata. Visual contact-sheet review found no page-level clipping or overflow. With hosted artifact verification complete, the one-off acceptance/migration workflow branches, admin audit restoration, and `scripts/tmp-*` helpers were removed; permanent application fixes remain.
