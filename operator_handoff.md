...
- **Scope / files changed:** reconciled five stale source-text tests with the shared presentation-only workflow wording module (`tests/durable-ai-analyze-workflow.test.ts`, `tests/manual-button-visibility.test.ts`, `tests/status-panels-do-not-report-stale-pending.test.ts`) and recorded this handoff. Production behavior and all workflow authorities/gates are unchanged.
- **Tests / CI:** both exact-head real-PostgreSQL CI runs reached 9,848/9,853 before the same five stale assertions failed; their failures were inspected and reproduced. The reconciled focused set passes 54/54 locally. Local production `npm run build` passes with development-only missing-provider/worker/Sentry warnings. Prisma generation, typecheck, lint, full real-PostgreSQL suite, build, browser isolation, and the exact-head screenshot audit must pass again on the new commit before completion is claimed.
...
- **Next action:** push, require fresh exact-head CI and screenshot artifacts, inspect the rendered original contradiction state, and repair any genuine remaining failure.
- **Merge status:** DO NOT MERGE — draft until all fresh exact-head checks and rendered evidence pass.
