...
- **Scope / files:** added durable server-side upload-first AI enqueue (`lib/engine/server-side-ai-enqueue.ts`, `lib/tender-upload-first.ts`, new-tender UI); replaced request-time whole-vault ingestion with selected-package `VAULT_INGEST` jobs (`lib/secure-upload-handler.ts`, company importers, job types/policy/handler); added six independent capability readiness results and admin UI (`lib/engine/capability-readiness.ts`, admin AI-readiness page); added bounded fail-closed retry classification for `EXTRACT_TEXT`, `VAULT_INGEST`, and `ENGINE_RUN`, due-time-aware claims, and privacy-allowlisted stage telemetry (`lib/engine/stage-retry-policy.ts`, `lib/engine/stage-observability.ts`, run-next/claim code); added `tests/durable-upload-orchestration.test.ts`.
- **Gap recheck:** Gap 2 (server-side enqueue) and Gap 5 (capability-specific UI) are implemented. Gap 1 is partial: only `server-side-ai-enqueue.ts`, `capability-readiness.ts`, and the durable-upload test are present; nine requested authority modules and the 84-test authority suite are absent because the closed refs/diffs are unavailable. Gap 3 is partial: selected-package `VAULT_INGEST` replaces request-time whole-vault ingestion, but no first-class ingestion-revision model exists. Gap 6 is partial: bounded due-time retry exists, but there is no new per-stage content-checkpoint model. Gap 8 is partial: the allowlisted telemetry contract exists and generic successful jobs emit it, but not every stage/artifact boundary emits it. Gaps 4, 7, and 9 remain external/unverified.
- **Tests:** targeted orchestration + release snapshot tests passed (31/31); Prisma generate and typecheck passed; lint passed with zero warnings; release-integrity audit passed; the production build completed successfully with non-secret placeholder environment values. `git diff --check` passed.
- **External/unavailable acceptance items:** no Git remote, `gh`, closed PR refs, Vercel credentials/secrets, preview URL, production log access, or PostgreSQL service are available. Consequently the exact #1260/#1261 source could not be reviewed/recreated, Z.ai/OpenRouter secrets were not configured, PR #1175 was not changed from draft, and live CI/preview/version/log/health/237-screenshot/full-DB verification was not performed or claimed. Production was not changed.
- **Known risks:** `VAULT_INGEST` uses the existing `AiJob` retry/checkpoint fields rather than a new migration, avoiding production migration risk. The exact closed-workstream acceptance modules remain absent because repository policy forbids reconstructing/copying unreviewed PR code. A reviewer with GitHub/Vercel/PostgreSQL access must complete those external/source-dependent items.
- **Next action:** fetch and review the actual #1260/#1261 diffs onto this branch, run the full disposable-PostgreSQL/Playwright/screenshot/preview suite, configure missing provider secrets through approved Vercel secret handling, then mark #1175 ready only after green CI.
- **Merge status:** unsafe / incomplete final acceptance; do not merge or deploy.
