...
async function syncAnalysisChunksFromOrchestrator(jobId: string): Promise<void> {
  const job = await prisma.aiJob.findUnique({ where: { id: jobId }, select: { output: true } });
  if (!job) return;
  ...
  const chunks = await prisma.aiAnalyzeChunk.findMany({ where: { jobId }, select: { id: true, chunkIndex: true } });
  for (const chunk of chunks) {
    const cr = chunkResults.find((r) => r.index === chunk.chunkIndex);
    if (cr) {
      await prisma.aiAnalyzeChunk.update({
        where: { id: chunk.id },
        data: { status: "SUCCEEDED", finishedAt: new Date(), provider: typeof cr.provider === "string" ? cr.provider : null, resultJson: JSON.stringify(cr.result) },
      });
    } else {
      await prisma.aiAnalyzeChunk.update({
        where: { id: chunk.id },
        data: { status: "FAILED", finishedAt: new Date(), failureCategory: "PROVIDER_EXHAUSTED", errorMessage: errorMessage ?? "Chunk not completed by orchestrator" },
      });
    }
  }
}
...
const handlers: Partial<Record<JobType, JobHandler>> = {
  ...
  // The SINGLE production path for AI Analyze. The user-facing button
  // enqueues via POST /api/tenders/[id]/ai-analyze?mode=background (202),
  // then triggers /api/ai-jobs/run-next?jobType=AI_ANALYZE to start this
  // handler.
  //
  // CONTRACT:
  //   • Calls executeAnalysis() for chunked AI analysis with provider
  //     fallback (Anthropic always last).
  //   • On FULL AI success: calls finalizeAnalysisJob() to promote
  //     canonical requirements + metadata BEFORE declaring success.
  //   • On partial/failed/regex-fallback/provider-exhausted: preserves
  //     PARTIAL_SUCCESS or FAILED, does NOT call the finalizer, does NOT
  //     overwrite with SUCCEEDED, does NOT create GeneratedDocument rows.
  //   • Returns { terminalStatus, ... } so run-next respects the terminal.
    ...
    try {
      ...
      if (result.success && !result.isPartial && result.analysisSource === "AI") {
        ...
        try {
          ...
          await recordStep(ctx.jobId, { stepName: "analyze.finalize_failed", message: `Finalization failed: ${finErr instanceof Error ? finErr.message : String(finErr)}`, status: "FAILED" }).catch(() => {});
        }

        if (finalizationStatus === "SUCCEEDED") {
          await recordStep(ctx.jobId, { stepName: "analyze.complete", message: `Analysis complete — ${result.requirementCount} requirements extracted and promoted to canonical`, status: "SUCCEEDED" });
        }

        return {
          terminalStatus: finalizationStatus, finalizationCode,
          analysisSource: result.analysisSource, requirementCount: result.requirementCount,
          isPartial: finalizationStatus !== "SUCCEEDED", success: finalizationStatus === "SUCCEEDED",
          totalChunks: result.totalChunks, completedChunks: result.completedChunks,
          failedChunks: result.failedChunks, jobId: result.jobId, errorMessage: result.errorMessage,
        };
      }
      ...
      await recordStep(ctx.jobId, {
        ...
        message: terminalStatus === "PARTIAL_SUCCESS"
      ...
      return {
        terminalStatus, analysisSource: result.analysisSource,
        requirementCount: result.requirementCount, isPartial: result.isPartial,
        success: false, totalChunks: result.totalChunks, completedChunks: result.completedChunks,
        failedChunks: result.failedChunks, jobId: result.jobId, errorMessage: result.errorMessage,
      };
    } catch (err) {
      clearInterval(heartbeat);
