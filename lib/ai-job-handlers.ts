...
const handlers: Partial<Record<JobType, JobHandler>> = {
  ...
  // ─── AI_ANALYZE — runs the tender engine in analyze-only mode ─────────
  // Same engine pipeline as ENGINE_RUN but skips post-match steps so it
  // returns as soon as requirements are extracted. Used by the
  // /api/tenders/[id]/ai-analyze route to escape the 60s request cap.
