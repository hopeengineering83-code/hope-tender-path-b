...
const handlers: Partial<Record<JobType, JobHandler>> = {
  ...
  // CRITICAL FIX (investigation FM-003): the AI_ANALYZE job type was listed
  // in SUPPORTED_JOB_TYPES but had NO handler registered, so the worker
  // would fail any AI_ANALYZE job with "No handler registered for
  // jobType=AI_ANALYZE". This handler delegates to runTenderEngine (which
  // includes AI analysis as its first stage), making AI_ANALYZE jobs
  // processable via the job queue.
