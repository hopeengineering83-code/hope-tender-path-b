...
async function bootstrap(client: PrismaClient): Promise<void> {
  ...
  // ── SubmissionPlanRevision / SubmissionPlanItem / SubmissionPlanItemRequirement /
  // SubmissionPlanItemCitation / TenderMetadataEvidence / RequirementEvidenceDecision
  // (PR #923 — persisted submission plan + evidence system)
  ...
  await client.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "RequirementEvidenceDecision" (
    ...
    FOREIGN KEY ("expertId") REFERENCES "Expert"("id") ON DELETE RESTRICT,
    FOREIGN KEY ("projectId") REFERENCES "Project"("id") ON DELETE RESTRICT,
    FOREIGN KEY ("legalRecordId") REFERENCES "LegalRecord"("id") ON DELETE RESTRICT,
    FOREIGN KEY ("companyComplianceRecordId") REFERENCES "CompanyComplianceRecord"("id") ON DELETE RESTRICT,
    FOREIGN KEY ("companyAssetId") REFERENCES "CompanyAsset"("id") ON DELETE RESTRICT,
    FOREIGN KEY ("originalUploadId") REFERENCES "CompanyAsset"("id") ON DELETE RESTRICT
  )`);
