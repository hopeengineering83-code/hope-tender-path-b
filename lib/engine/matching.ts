...
export function buildMatches(
  ...
  const expertMatches = knowledge.experts
    .map((expert, idx) => {
      ...
      const topMatches = [...new Set(docTokens.filter((t) => queryTokens.includes(t)))].slice(0, 4).join(", ");
      return {
        expertId: expert.id,
        score,
        rationale: score > 0.02 ? `Matched on: ${topMatches || evidence}.${expert.yearsExperience ? ` ${expert.yearsExperience} yrs experience.` : ""}` : "Limited keyword overlap with this tender.",
  ...
  const projectMatches = knowledge.projects
    .map((project, idx) => {
      ...
      return {
        ...
        score,
        rationale: score > 0.02 ? `Matched on: ${topMatches || evidence}.${project.contractValue ? ` Contract: ${project.currency ?? "USD"} ${project.contractValue.toLocaleString()}.` : ""}` : "Limited project overlap with this tender.",
        evidenceSummary: evidence || "No evidence summary",
        isSelected: false,
      };
    })
