...
import { extractDeepTenderComprehension, formatComprehensionForPrompt, type DeepTenderComprehension } from "../analysis/evaluation-criteria-extractor";
import { runDeepRefinement } from "../deep-reasoning/deep-reasoning-refiner";
import { alignMatchesToEvaluatorCriteria, formatAlignmentForPrompt, type AlignmentCandidate, type AlignmentReport } from "../matching/semantic-match-aligner";
import { executeProposalTool, PROPOSAL_TOOL_DEFS, type ToolEvidenceInventory } from "./proposal-tools";
import { DeepReasoningTelemetry } from "../deep-reasoning/deep-reasoning-telemetry";
...
import { enforceCanonicalNames } from "../metadata/entity-name-normalizer";
import { exactSelectionLimit, forbidsBranding, forbidsCoverPage, requiresSignatureOrStamp } from "../workflow/scope-policy";
import { finalizeClientReadyProposalMarkdown } from "../quality/proposal-benchmark-guard";
import { appendEvaluatorResponseMatrix } from "../evaluation/proposal-evaluator-matrix";
import { buildClientProposalStrengtheningSections } from "./proposal-strengthening-sections";
import { benchmarkAuditSummary } from "../quality/proposal-benchmark-audit";
import { polishBenchmarkOutput } from "./benchmark-output-polisher";
import { formatRequirementLine } from "../infrastructure/proposal-labels";
import {
...
import { computeWinProbability, formatWinProbability } from "../evaluation/win-probability";
...
import { buildComplianceMatrixSection, hasComplianceMatrixHeading } from "../compliance/compliance-matrix-builder";
import { buildEvaluatorMirrorSection, hasEvaluatorMirrorHeading } from "../evaluation/evaluator-mirror-builder";
...
import { extractTenderFacts, formatFactsForPrompt, buildTenderSpecificsBlock } from "../analysis/tender-facts-extractor";
import { formatQualityScoreSummary, scoreProposalQuality } from "../quality/proposal-quality-scorer";
import {
...
import { enforceClientName } from "../metadata/client-name-enforcer";
...
import { deduplicateTables, injectQaThresholds, injectAppendixReadinessRegister } from "../quality/advanced-quality-passes";
import { generateExpertCvDocx, expertCvFileName } from "./expert-cv-docx";
import { applyProposalQualityRepairAddenda } from "../quality/proposal-quality-repair";
import { computeBidStrategy } from "../workflow/bid-strategy";
import { applyAIWriterContractPrompt } from "./ai-writer-contract-prompt";
import type { TenderSourceDocument } from "../compliance/source-grounded-requirement-map";
...
export async function generateTenderDocuments(tenderId: string, userId: string): Promise<void> {
  ...
  try {
    const { extractCanonicalTenderTitle, pickBestTenderTitle } = await import("../analysis/tender-title-extractor");
  ...
  if (isAIEnabled() && (experts.length > 0 || projects.length > 0)) {
    try {
      const { aiRematchExperts, aiRematchProjects } = await import("../matching/ai-multi-perspective-matcher");
  ...
  try {
    const { injectDeliverableQaChecklist } = await import("../quality/deliverable-qa-checklist");
  ...
  try {
    const { writeSectionEvidenceFromMarkdown } = await import("../infrastructure/section-evidence-map");
