...
export async function generateTenderDocuments(tenderId: string, userId: string): Promise<void> {
  ...
  try {
    ...
    void reportError(tErr, { module: "generate-elite" });
    // Non-critical: stored title is still usable. Log and continue.
    logger.warn("[generate-elite] tender-title-extractor failed:", { error: tErr instanceof Error ? tErr.message : tErr });
  }
  ...
  if (useDeepReasoning) {
    try {
      ...
      void reportError(compErr, { module: "generate-elite" });
      logger.warn("[generate-elite] Deep comprehension threw (non-critical):", { error: compErr instanceof Error ? compErr.message : compErr });
    }
  }
  ...
  if (isAIEnabled() && (experts.length > 0 || projects.length > 0)) {
    try {
      ...
      void reportError(err, { module: "generate-elite" });
    ...
    try {
      ...
      if (useDeepReasoning && deepComprehension && deepComprehension.criteria.length > 0) {
        try {
          ...
          void reportError(alignErr, { module: "generate-elite" });
          logger.warn("[generate-elite] Semantic alignment threw (non-critical):", { error: alignErr instanceof Error ? alignErr.message : alignErr });
        }
      }
      ...
      void reportError(error, { module: "generate-elite" });
  ...
  try {
    ...
    void reportError(qaErr, { module: "generate-elite" });
    logger.warn("[generate-elite] Deliverable QA Checklist injection failed:", { error: qaErr instanceof Error ? qaErr.message : qaErr });
  }
  ...
  if (
    ...
    try {
      ...
      void reportError(deepErr, { module: "generate-elite" });
  ...
  if (deepComprehension || alignmentReport || deepRefinementApplied || deepTelemetry.getRecords().length > 0) {
    try {
      ...
      void reportError(auditErr, { module: "generate-elite" });
  ...
  try {
    ...
    void reportError(vErr, { module: "generate-elite" });
    // Version saving is non-critical — never block the main proposal.
    logger.warn("[generate-elite] Proposal version snapshot failed:", { error: vErr instanceof Error ? vErr.message : vErr });
  }
  ...
  try {
    ...
    void reportError(sErr, { module: "generate-elite" });
    logger.warn("[generate-elite] Section evidence map write failed:", { error: sErr instanceof Error ? sErr.message : sErr });
  }
