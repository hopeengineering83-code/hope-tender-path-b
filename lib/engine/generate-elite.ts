...
import { AlignmentType, BorderStyle, Document, Footer, Header, HeadingLevel, Packer, PageNumber, Paragraph, Table, TableBorders, TableCell, TableOfContents, TableRow, TextRun, WidthType } from "docx";
...
export function markdownToDocx(markdown: string): (Paragraph | Table | TableOfContents)[] {
  ...
  for (const raw of markdown.replace(/\r/g, "").split("\n")) {
    ...
    // A markdown TOC is useful before rendering, but a final Word document
...
export function buildProfessionalDocument(params: {
  ...
  return new Document({
    ...
    // Mark dynamic fields dirty so Word refreshes the TOC and page references
    // on open rather than preserving stale cached entries.
