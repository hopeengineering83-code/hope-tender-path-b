...
export function runAuthorityReview(
  ...
  // Required sections are usually the tender's exact submission file names.
  // Compare on the BASE name (extension stripped) and include the document's
  // own exactFileName in the comparison: generated rows are named without the
  // extension ("01-Expression-Of-Interest"), so a required section of
  // "01-Expression-Of-Interest.docx" matched neither `name` nor `documentType`
  // and every correctly generated file was reported as a CRITICAL
  // MISSING_REQUIRED_SECTION, blocking the download with
