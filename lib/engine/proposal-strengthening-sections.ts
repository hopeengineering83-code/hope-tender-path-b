...
function sectorMethodology(input: ProposalStrengtheningInput): string[] {
  ...
  // Word boundaries on bare abbreviations — see proposal-intelligence.ts
  // for the full rationale (bare /MIS/i matched "optimISation" etc.).
  if (/\bICT\b|software|system|digital|database|platform|telecom|network|cyber|\bERP\b|\bMIS\b/i.test(text)) return ["ICT / digital-service tenders: address user requirements, business process review, system architecture, data/security controls, integrations, implementation roadmap, testing and acceptance, training, support model, documentation, risk controls, service continuity and handover.", "Methodology should map requirements to modules, deliverables, acceptance criteria, implementation phases, governance and support responsibilities."];
