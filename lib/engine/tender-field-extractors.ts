...
export function extractReference(input: ExtractorInput): ExtractedFieldOrMissing<string> {
  ...
  for (const file of input.files ?? []) {
    ...
    for (const p of REFERENCE_PATTERNS) {
      ...
      if (!m) continue;
      cands.push({ found: true, value: m[1].trim(), sourceQuote: captureAround(text, m.index, m[0].length), sourceFile: file?.fileName ?? null, sourcePage: getSourcePage(text, m.index, file?.totalPages), confidence: p.confidence });
      break;
    }
  }
