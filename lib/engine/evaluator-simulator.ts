...
async function runPersona(persona: EvaluatorPersona, systemPrompt: string, userPrompt: string): Promise<PersonaAssessment | null> {
  ...
  try {
    ...
    void reportError(err, { module: "evaluator-simulator" });
    logger.warn(`[evaluator-simulator] Persona ${persona} call failed:`, { error: err instanceof Error ? err.message : err });
    return null;
  }
}
