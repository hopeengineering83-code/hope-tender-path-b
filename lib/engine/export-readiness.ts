...
export function deriveTenderLevelExportHardBlockers(input: {
  ...
  if (input.activeDocuments === 0) {
    blockers.push({ category: "EXPORT_BASELINE", severity: "HIGH", title: "NO_ACTIVE_GENERATED_DOCUMENTS", recommendedAction: "Generate required tender documents before attempting export." });
  }
  if ((input.tenderStage ?? "").toUpperCase() === "ANALYSIS" || (input.tenderStatus ?? "").toUpperCase() === "ANALYZED") {
    blockers.push({ category: "EXPORT_BASELINE", severity: "HIGH", title: "FULL_PROPOSAL_NOT_READY", recommendedAction: "Run Engine and generate/review proposal documents before final export." });
  }
  if ((input.readinessScore ?? 0) <= 0) {
    blockers.push({ category: "EXPORT_BASELINE", severity: "MEDIUM", title: "READINESS_SCORE_ZERO", recommendedAction: "Resolve readiness blockers before final export." });
  }
  return blockers;
}
...
export function checkExportReadiness(docs: ExportReadyDocument[], opts: { requireFileContent?: boolean } = {}): ExportReadinessResult {
  ...
  for (const doc of docs) {
    ...
    // Only apply the binary state machine when content actually looks like base64 binary.
    // Plain text, empty content, and stub values fall back to column-based checks so
    // legacy/intermediate documents are not retroactively blocked.
    const hasBinaryContent = doc.fileContent ? looksLikeBase64Payload(doc.fileContent) : false;
    if (hasBinaryContent) {
