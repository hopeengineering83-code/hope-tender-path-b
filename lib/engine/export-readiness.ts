...
function documentLabel(doc?: Pick<ExportReadyDocument, "name" | "exactFileName" | "documentType" | "format">): string {
...
function containsSubstantivePricingLeak(text: string, doc?: Pick<ExportReadyDocument, "name" | "exactFileName" | "documentType" | "format">): boolean {
  if (isFinancialEvidenceDocument(doc)) return false;

  const candidateText = pricingScanSentences(text)
    .filter((sentence) => !isNoPriceControlSentence(sentence))
    .join(" ");
...
function documentHygieneIssues(text: string | null | undefined, doc?: Pick<ExportReadyDocument, "name" | "exactFileName" | "documentType" | "format">): string[] {
