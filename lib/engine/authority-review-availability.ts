...
export function resolveAuthorityReviewAvailability(
  ...
  if (input.requiredDocumentCount <= 0) {
    return {
      ...
      reason: "Build the tender-controlled document plan before Authority Review.",
    };
  }
  if (input.generatedDocumentCount <= 0 || input.missingRequiredCount > 0) {
    return {
      ...
      reason: "Generate every required document before Authority Review.",
    };
  }
  if (input.validDocumentCount < input.requiredDocumentCount) {
    return {
      ...
      reason: "Validate every required document before Authority Review.",
    };
  }
