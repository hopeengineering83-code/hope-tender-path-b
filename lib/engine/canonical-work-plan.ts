...
function sectorPhasingRows(sector: string): PhasingRow[] {
  ...
  if (/spectrum|broadband|LTE|5G|base.*station|backhaul|mobile.*network/i.test(s)) {
    return [
      ...
      { phase: "3. Procurement & Site Works", deliverables: "Tender documents (quantity schedules, specs); in-principle spectrum approval confirmation; site acquisition agreements; installation supervision", duration: "Weeks 11–18", responsible: "Site Acquisition Coordinator" },
  ...
  if (/interior design|fit[-\s]?out|space planning|finishes.*schedule|furniture.*layout|joinery/i.test(s)) {
    return [
      ...
      { phase: "2. Concept Design", deliverables: "Concept boards (mood, material palette, lighting concept); furniture concept layouts; preliminary quantity schedules (indicative)", duration: "Weeks 3–5", responsible: "Interior Designer" },
      ...
      { phase: "4. Detailed Design & FF&E Specification", deliverables: "Full FF&E schedule with supplier options and lead times; room data sheets; MEP coordination drawings; construction documentation package (drawings, specs, quantity schedules)", duration: "Weeks 10–15", responsible: "Lead Interior Architect" },
  ...
  if (/industrial|manufactur|factory|abattoir|processing.*plant|production.*facilit|warehouse.*industrial/i.test(s)) {
    return [
      ...
      { phase: "4. Tender Documents & Procurement", deliverables: "Full tender package (quantity schedules, drawings, specs); equipment procurement list; factory acceptance test (FAT) requirements", duration: "Weeks 17–20", responsible: "Lead Engineer" },
  ...
  if (/hotel|hospitality|resort|lodge|guesthouse|five.star|luxury.*accommodat/i.test(s)) {
    return [
      { phase: "1. Concept & Feasibility", deliverables: "Feasibility study (market demand, RevPAR analysis, development program); concept design (room mix, F&B, BOH layout); landscape/pool/spa concept; preliminary quantity schedules", duration: "Weeks 1–4", responsible: "Lead Architect + Project Principal" },
      ...
      { phase: "4. Tender Documents & Procurement", deliverables: "Full tender package (drawings, quantity schedules, specs); FF&E procurement schedule; brand operator sign-off; construction supervision plan", duration: "Weeks 19–22", responsible: "Lead Architect + QS" },
...
/**
 * The engagement's total duration in days, when the tender states one
 * ("28 calendar days from signed contract"), otherwise undefined.
 *
 * Lives here because it is a property of the work plan, and two callers
 * parsing it independently is how the plan came to speak weeks in one place
 * and days in another. Returns undefined outside 7..1000 days: below that the
 * number is almost always a notice period rather than a programme, and above
 * it a page or reference number.
 */
export function tenderTotalDays(tenderText: string): number | undefined {
  const match = tenderText.match(
    /\b(\d{1,3})\s*(?:calendar\s+|working\s+|business\s+)?days?\b(?!\s*(?:after|before|prior|notice|advance))/i,
  );
  if (!match) return undefined;
  const days = Number(match[1]);
  return days >= 7 && days <= 1000 ? days : undefined;
}
