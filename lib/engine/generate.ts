...
function buildDeclarationContent(companyName: string, tenderTitle: string): Paragraph[] {
  return [heading1("Declaration"), body(`We, the undersigned, being duly authorized representatives of ${companyName}, hereby declare the following:`, { bold: true }), body(`1. We confirm that all information provided in this proposal for \"${tenderTitle}\" is accurate and complete to the best of our knowledge.`), body("2. We confirm that our organization is not debarred, suspended, or otherwise ineligible to participate in the tendering process."), body("3. We confirm that we have read and understood all terms, conditions, and requirements of this tender."), body("4. We confirm that we have no conflict of interest in relation to this assignment."), body("5. We commit to maintaining the confidentiality of all information received in connection with this tender."), new Paragraph({ spacing: { before: 480, after: 60 } }), body("Authorized Signatory:", { bold: true }), body("Name: _______________________________"), body("Title: _______________________________"), body("Signature: ___________________________"), body("Date: ________________________________"), body("Company Seal / Stamp:")];
}
...
function buildCompanyProfileContent(company: { name: string; legalName?: string | null; description?: string | null; profileSummary?: string | null; address?: string | null; phone?: string | null; email?: string | null; website?: string | null; serviceLines: string; sectors: string }): Paragraph[] {
  const paras: Paragraph[] = [];
  paras.push(heading1("Company Profile")); paras.push(heading2(company.name));
...
function buildDocxFromParagraphs(paragraphs: Paragraph[], companyName: string, docTitle: string): Document {
  return new Document({ sections: [{ properties: {}, children: paragraphs, headers: { default: new Header({ children: [new Paragraph({ children: [new TextRun({ text: companyName, size: 18, color: "777777" })], alignment: AlignmentType.RIGHT })] }) }, footers: { default: new Footer({ children: [new Paragraph({ children: [new TextRun({ text: `${docTitle} — Confidential`, size: 16, color: "999999" })], alignment: AlignmentType.CENTER })] }) } }], styles: { default: { document: { run: { font: "Calibri", size: 22 }, paragraph: { spacing: { line: 276 } } } } } });
}

function takeNext<T>(items: T[], state: { index: number }): T | null {
...
export async function generateTenderDocuments(tenderId: string, userId: string): Promise<void> {
  const tender = await prisma.tender.findFirst({ where: { id: tenderId, userId }, include: { requirements: true, expertMatches: { where: { isSelected: true }, include: { expert: true } }, projectMatches: { where: { isSelected: true }, include: { project: true } }, generatedDocuments: true } });
  ...
  const plannedDocs = tender.generatedDocuments.filter((d) => d.generationStatus === "PLANNED").sort((a, b) => (a.exactOrder ?? 999) - (b.exactOrder ?? 999));
  const hasProposal = plannedDocs.some((d) => ["PROPOSAL", "TECHNICAL_PROPOSAL", "METHODOLOGY"].includes(d.documentType));
  const docsToGenerate = hasProposal ? plannedDocs : [{ id: null, name: `${tender.title} — Technical Proposal`, documentType: "TECHNICAL_PROPOSAL", exactFileName: `${tender.title.replace(/[^a-zA-Z0-9]/g, "-")}-Technical-Proposal.docx`, exactOrder: 1, contentSummary: null }, ...plannedDocs];
  const letterheadParas = buildLetterheadHeader(company); const coverParas = buildCoverSection(tender.title, tender.reference, tender.clientName, company.name);
  const expertState = { index: 0 }; const projectState = { index: 0 };

  for (const doc of docsToGenerate) {
    let contentParagraphs: Paragraph[] = []; let docTitle = doc.name;
    try {
      if (["TECHNICAL_PROPOSAL", "PROPOSAL", "METHODOLOGY"].includes(doc.documentType)) {
        ...
        const expert = takeNext(selectedExperts, expertState);
        if (expert) { contentParagraphs = [...letterheadParas, ...buildCVContent(expert)]; docTitle = `CV — ${expert.fullName}`; }
      } else if (doc.documentType === "PROJECT_EXPERIENCE") {
        const project = takeNext(selectedProjects, projectState);
        if (project) { contentParagraphs = [...letterheadParas, ...buildProjectReferenceContent(project)]; docTitle = `Project Reference — ${project.name}`; }
      } else if (doc.documentType === "DECLARATION") {
        contentParagraphs = [...letterheadParas, ...buildDeclarationContent(company.name, tender.title)]; docTitle = "Declaration";
      } else if (doc.documentType === "COMPANY_PROFILE") {
        contentParagraphs = [...letterheadParas, ...buildCompanyProfileContent(company)]; docTitle = "Company Profile";
      } else {
