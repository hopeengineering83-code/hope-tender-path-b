...
export function evaluateSevenPassGenerationGate(input: GeneratedDocumentGateInput): SevenPassEvaluation {
  ...
  passes.push(result("TENDER_COMPREHENSION", [
    ...
    ...(analysisSource === "REGEX_FALLBACK" ? ["Tender was analyzed by unapproved regex fallback and needs AI analysis before final output."] : []),
    // PERMANENT BLOCK: HUMAN_APPROVED_REGEX is audit-only — it must NEVER
    // authorize final senior-quality approval. Re-run AI Analyze to obtain a
    // genuine AI analysis before final output.
  ...
  passes.push(result("FINAL_REWRITE_APPROVAL", [
    ...
    // PERMANENT BLOCK: HUMAN_APPROVED_REGEX is audit-only — must NEVER
    // authorize FINAL_REWRITE_APPROVAL.
    ...(analysisSource === "HUMAN_APPROVED_REGEX" ? ["Human-approved regex fallback is audit-only and cannot authorize final rewrite approval. Re-run AI Analyze."] : []),
  ]));

  const blockers = Array.from(new Set(passes.flatMap((pass) => pass.blockers)));
  const finalApprovalAllowed = blockers.length === 0;
  // HUMAN_APPROVED_REGEX is now blocked the same as REGEX_FALLBACK — audit-only.
  const isFallbackLike = deterministicFallbackUsed || analysisSource === "REGEX_FALLBACK" || analysisSource === "HUMAN_APPROVED_REGEX";

  return {
    finalApprovalAllowed,
    recommendedValidationStatus: finalApprovalAllowed ? "PASSED" : isFallbackLike ? "DRAFT" : "BLOCKED",
