...
Tender title: ${input.tenderTitle}
...
export function buildQuickDraftFallback(input: QuickDraftInput, reason: "AI_DISABLED" | "AI_UNAVAILABLE"): string {
  const notice = reason === "AI_DISABLED"
    ? "> Draft generated by deterministic benchmark fallback because the AI writer is not configured."
    : "> Draft generated by deterministic benchmark fallback because the AI writer was temporarily unavailable. Provider error details are hidden from the proposal text.";
  ...
  return [
    ...
    "# Cover Letter",
    `To: ${input.clientName}`,
    `${input.companyName} submits this technical proposal draft for ${input.tenderTitle}. The final proposal should lead with verified comparable project evidence, selected CVs, and tender-specific methodology before submission.`,
    "",
    "# Technical Proposal",
