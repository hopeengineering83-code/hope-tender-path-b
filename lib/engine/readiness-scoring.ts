...
export function computeReadinessScore(input: ReadinessScoreInput): ReadinessScoreResult {
  ...
  if (input.analysisSource === "HUMAN_APPROVED_REGEX_FALLBACK") applicableCaps.push({ dimension: "analysisSource", capScore: 70, reason: "Human-approved regex fallback is draft-review only unless explicit final admin override exists." });
