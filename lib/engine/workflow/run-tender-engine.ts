...
import { analyzeTender, normalizeStrategicRequirements } from "../analysis/analysis";
import { analyzeWithAI, isAIEnabled } from "../ai";
import { buildCompliance } from "../compliance/compliance";
import { buildDocumentPlan } from "../documents/documents";
import { buildMatches } from "../matching/matching";
import { applyMainEngineBestAvailableSelection } from "../matching/main-engine-selection-policy";
import { applyAIRematchToMainEngine } from "../matching/main-engine-ai-rematch";
import type { MatchPerspective } from "../matching/ai-multi-perspective-matcher";
import { inferSector } from "../generation/proposal-intelligence";
...
export async function runTenderEngine(
  ...
  try {
    ...
    if (filesWithText.length > 0 && analysis.requirements.length > 0) {
      try {
        const { extractRequirementSources } = await import("../analysis/requirement-source-extractor");
    ...
    try {
      const { writeScoreBreakdown, deterministicScoreBreakdown } = await import("../generation/score-breakdown-writer");
