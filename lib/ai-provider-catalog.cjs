...
// Zero-paid mode is an immutable deployment policy, not an operator toggle.
// Retaining the argument keeps the shared API backwards-compatible, but even a
// stale Vercel override such as AI_ZERO_PAID_MODE=false must fail closed rather
// than making paid providers reachable.
...
function automaticProviderOrder(env) {
  return ZERO_PAID_AUTOMATIC_ORDER;
}
