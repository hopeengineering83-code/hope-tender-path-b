...
// The provider order the automatic fallback chain uses. It is the canonical
// order, unconditionally: no mode, flag or cost class reorders or filters it.
// The function is kept (rather than inlining the constant) because every
// consumer already reads the chain through it, and that single accessor is what
// makes "there is one order" checkable.
function automaticProviderOrder() {
