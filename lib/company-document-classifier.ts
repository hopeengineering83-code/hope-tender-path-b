...
const EXPERT_PATTERNS = [/\bcurriculum\s*vitae\b/i, /\bcv\b\s*(of|for|:)/i, /\bname\s+of\s+(expert|key\s+staff|personnel)\b/i, /\bproposed\s+(position|role)\b/i, /\byears?\s+of\s+experience\b/i, /\bqualifications?\s*:/i, /\bkey\s+(staff|personnel|experts?)\b/i];
const PROJECT_PATTERNS = [/\bproject\s+(name|title|description)\b/i, /\bclient\s+name\b/i, /\bcontract\s+(value|amount)\b/i, /\bselected\s+projects?\b/i, /\bpast\s+projects?\b/i, /\bportfolio\b/i];
function matchAny(text: string, patterns: RegExp[]): boolean { return patterns.some((p) => p.test(text)); }
export function classifyDocumentContent(fileName: string, category: string, extractedText: string | null | undefined): DocumentClassification {
  const text = extractedText ?? "";
  const hasExpertData = matchAny(text, EXPERT_PATTERNS) || /cv|expert|staff|resume|personnel|curriculum/i.test(fileName);
  const hasProjectData = matchAny(text, PROJECT_PATTERNS) || /project|portfolio|reference|contract/i.test(fileName);
