...
export function deriveProviderStatus(provider: AiProviderName): AiProviderStatus {
  ...
  // Configuration validity checks (OpenRouter / Z.ai). These check for invalid
  // models or endpoint mismatches that would cause HTTP 400 errors.
  // Configuration problems never consume an attempt budget slot.
  ...
  if (provider === "zai") {
    const validity = zaiConfigurationValidity();
    if (!validity.valid) {
      if (validity.reason === "MODEL_ENDPOINT_MISMATCH") return "CONFIGURATION_INVALID";
      if (validity.reason === "MODEL_UNAVAILABLE") return "MODEL_UNAVAILABLE";
