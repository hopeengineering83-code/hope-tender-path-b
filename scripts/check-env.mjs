...
const PROVIDER_KEY_META = {
  ZAI_API_KEY: {
    description: `Z.ai GLM API key. Manual diagnostics/adapters only; not part of the automatic fallback chain (${CANONICAL_CHAIN}). General OpenAI-compatible endpoint (ZAI_BASE_URL, default https://api.z.ai/api/paas/v4). Models override via ZAI_PROPOSAL_MODEL / ZAI_ANALYSIS_MODEL / ZAI_FAST_MODEL (default glm-4-flash).`,
    validate: (_v) => null,
  },
  CEREBRAS_API_KEY: {
    description: `Cerebras API key. Manual diagnostics/adapters only; not part of the automatic fallback chain (${CANONICAL_CHAIN}). OpenAI-compatible endpoint that uses max_completion_tokens. Models override via CEREBRAS_PROPOSAL_MODEL / CEREBRAS_ANALYSIS_MODEL / CEREBRAS_FAST_MODEL (default gpt-oss-120b).`,
    validate: (_v) => null,
  },
  MISTRAL_API_KEY: {
    description: `Mistral API key. Manual diagnostics/adapters only; not part of the automatic fallback chain (${CANONICAL_CHAIN}). Models override via MISTRAL_PROPOSAL_MODEL / MISTRAL_ANALYSIS_MODEL / MISTRAL_FAST_MODEL.`,
    validate: (_v) => null,
  },
  GROQ_API_KEY: {
    description: `Groq API key (gsk_...). FOURTH-tier automatic AI provider in the canonical chain (${CANONICAL_CHAIN}). Model overridable via GROQ_PROPOSAL_MODEL (default llama-3.3-70b-versatile).`,
    validate: (_v) => null,
  },
  OPENROUTER_API_KEY: {
  ...
  GEMINI_API_KEY: {
    description: `Google Gemini API key (AIza... legacy or AQ... new format). FIRST-tier automatic AI provider in the canonical chain (${CANONICAL_CHAIN}). Without any automatic AI provider key, imported records remain REGEX_DRAFT only.`,
    validate: (v) => {
  ...
  OPENAI_API_KEY: {
    description: `OpenAI API key (sk-...). THIRD-tier automatic AI provider in the canonical chain (${CANONICAL_CHAIN}). At least one automatic AI provider key is required in production.`,
    validate: (v) => {
  ...
  TOGETHER_API_KEY: {
    description: `Together API key. Manual diagnostics/adapters only; not part of the automatic fallback chain (${CANONICAL_CHAIN}). Models override via TOGETHER_PROPOSAL_MODEL / TOGETHER_ANALYSIS_MODEL / TOGETHER_FAST_MODEL.`,
    validate: (_v) => null,
  },
  DEEPSEEK_API_KEY: {
    description: `DeepSeek API key. FIFTH-tier automatic fallback AI provider in the canonical chain (${CANONICAL_CHAIN}). OpenAI-compatible endpoint (deepseek-chat / deepseek-reasoner).`,
    validate: (_v) => null, // no canonical prefix to validate
  },
  ANTHROPIC_API_KEY: {
    description: `Anthropic Claude API key (sk-ant-..., 97+ chars). SIXTH-tier (last) automatic AI provider in the canonical chain (${CANONICAL_CHAIN}). Keep Claude last so earlier providers are preferred. Get from https://console.anthropic.com/settings/keys.`,
    validate: (v) => {
