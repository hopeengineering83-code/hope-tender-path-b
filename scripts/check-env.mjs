...
const AI_PROVIDER_KEYS = [
  {
    ...
    description:
      "Anthropic Claude API key (sk-ant-..., 97+ chars). EIGHTH-tier (last) AI provider in the canonical chain (Z.ai GLM → Cerebras → Mistral → Groq → OpenRouter → Gemini → OpenAI → Together → DeepSeek → Anthropic / Claude). Keep Claude last to avoid Anthropic rate limits blocking the app when other providers are available. Get from https://console.anthropic.com/settings/keys.",
    validate: (v) => {
  ...
  },
  {
    name: "GEMINI_API_KEY",
    description:
      "Google Gemini API key (AIza... legacy or AQ... new format). FOURTH-tier AI provider in the canonical chain (Z.ai GLM → Cerebras → Mistral → Groq → OpenRouter → Gemini → OpenAI → Together → DeepSeek → Anthropic / Claude). Without any AI provider key, imported records remain REGEX_DRAFT only.",
    validate: (v) => {
  ...
  },
  {
    name: "OPENAI_API_KEY",
    description:
      "OpenAI API key (sk-...). FIFTH-tier AI provider in the canonical chain (Z.ai GLM → Cerebras → Mistral → Groq → OpenRouter → Gemini → OpenAI → Together → DeepSeek → Anthropic / Claude). At least one AI provider key is required in production.",
    validate: (v) => {
  ...
  },
  {
    ...
    description:
      "Mistral API key. FIRST-tier (preferred) AI provider in the canonical chain (Z.ai GLM → Cerebras → Mistral → Groq → OpenRouter → Gemini → OpenAI → Together → DeepSeek → Anthropic / Claude). Used for analysis, extraction, proposal, validation, and fast use cases. Models override via MISTRAL_PROPOSAL_MODEL / MISTRAL_ANALYSIS_MODEL / MISTRAL_FAST_MODEL.",
    validate: (_v) => null,
  },
  {
    name: "DEEPSEEK_API_KEY",
    description:
      "DeepSeek API key. SEVENTH-tier fallback AI provider in the canonical chain (Z.ai GLM → Cerebras → Mistral → Groq → OpenRouter → Gemini → OpenAI → Together → DeepSeek → Anthropic / Claude). OpenAI-compatible endpoint (deepseek-chat / deepseek-reasoner).",
    validate: (_v) => null, // no canonical prefix to validate
  },
  {
    name: "GROQ_API_KEY",
    description:
      "Groq API key (gsk_...). SECOND-tier AI provider in the canonical chain (Z.ai GLM → Cerebras → Mistral → Groq → OpenRouter → Gemini → OpenAI → Together → DeepSeek → Anthropic / Claude). Fastest verified working provider. Model overridable via GROQ_PROPOSAL_MODEL (default llama-3.3-70b-versatile).",
    validate: (_v) => null,
  },
  {
    name: "TOGETHER_API_KEY",
    description:
      "Together API key. SIXTH-tier AI provider in the canonical chain (Z.ai GLM → Cerebras → Mistral → Groq → OpenRouter → Gemini → OpenAI → Together → DeepSeek → Anthropic / Claude). Models override via TOGETHER_PROPOSAL_MODEL / TOGETHER_ANALYSIS_MODEL / TOGETHER_FAST_MODEL.",
    validate: (_v) => null,
  },
  {
    name: "OPENROUTER_API_KEY",
    description:
      "OpenRouter API key (sk-or-...). THIRD-tier aggregator AI provider in the canonical chain (Z.ai GLM → Cerebras → Mistral → Groq → OpenRouter → Gemini → OpenAI → Together → DeepSeek → Anthropic / Claude). OpenAI-compatible endpoint. Model overridable via OPENROUTER_PROPOSAL_MODEL (default openrouter/auto).",
    validate: (_v) => null,
  },
];
