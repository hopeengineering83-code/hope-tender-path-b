// TEMPORARY — verified clean rebuild of a newly created Preview database.
// The target is named by EXPECTED_FINGERPRINT, never hardcoded, and the script
// refuses to touch any database whose fingerprint does not match it. Removed as
// part of cleanup once the recovery is complete and verified healthy.
//
// Reused 2026-09-07 for the second Neon switch (fingerprint 5f9645fb34f5),
// which reproduced the same signature as the first: 54 tables, no
// _prisma_migrations history, 430 drift statements against schema.prisma, and
// exactly 4 Role rows. Same cause, same gates, same procedure.
//
