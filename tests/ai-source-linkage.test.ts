...
test("exact tokens resolve files independently when page numbers overlap", () => {
  const a = resolveRequirementSourceFile(
    {
      sourceTenderFileId: null,
      sourceFileToken: "TFILE:file-a",
      sourceFileName: null,
      sourceQuote: "Submit the technical proposal by noon.",
      sourcePage: 12,
    },
    files,
  );
  const b = resolveRequirementSourceFile(
    {
      sourceTenderFileId: null,
      sourceFileToken: "TFILE:file-b",
      sourceFileName: null,
      sourceQuote: "Submit the financial proposal separately.",
      sourcePage: 12,
    },
    files,
  );
...
test("ambiguous duplicate names never fabricate a file id", () => {
  ...
  const result = resolveRequirementSourceFile(
    {
      sourceTenderFileId: null,
...
test("unique source quote recovers linkage but missing quote remains unresolved", () => {
  const linked = resolveRequirementSourceFile(
    {
      sourceTenderFileId: null,
      sourceFileToken: null,
      sourceFileName: null,
  ...
  const missing = resolveRequirementSourceFile(
    {
      sourceTenderFileId: null,
      sourceFileToken: null,
      sourceFileName: null,
      sourceQuote: "This quote does not exist in any tender file.",
      sourcePage: 4,
    },
    files,
  );
...
test("enrichment persists file id, method, and confidence", () => {
  const enriched = enrichRequirementsWithSourceLinkage(
    [
      {
        title: "Separate financial proposal",
        description: "Financial offer must be separate.",
        requirementType: "FINANCIAL",
        priority: "MANDATORY",
        sourceFileToken: "TFILE:file-b",
        sourceFileName: "Pricing.pdf",
        sourcePage: 12,
        sourceQuote: "Submit the financial proposal separately.",
      },
    ],
