...
describe("Company Vault automatic runtime authority", () => {
  ...
  it("refreshes Vault source links and verification before sync or async Engine execution", () => {
  ...
  it("keeps automatic promotion source-backed and fail-closed", () => {
    const authority = readFileSync("lib/vault-runtime-authority.ts", "utf8");
    const preflight = readFileSync("lib/engine/prepare-company-vault.ts", "utf8");
    assert.match(authority, /isDurablySourceVerified/);
    assert.match(authority, /isDurablyReviewed/);
