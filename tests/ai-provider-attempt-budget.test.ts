...
describe("12. only three actual outbound attempts occur per request", () => {
  it("makes at most 3 outbound provider calls even when 5+ are configured", async () => {
    ...
    assert.ok(m.calls() <= 3 && m.calls() >= 1, `expected at most 3 outbound attempts, got ${m.calls()}`);
  });
});
...
describe("11. cooldown providers are skipped without consuming the budget", () => {
  it("skips a cooled-down provider and still tries 3 live providers", async () => {
    process.env.GEMINI_API_KEY = "AQ-test-key-with-length";       // will be put in cooldown
    process.env.OPENROUTER_API_KEY = "sk-or-test";
    process.env.OPENAI_API_KEY = "sk-test";
    process.env.GROQ_API_KEY = "gsk_test";
    process.env.DEEPSEEK_API_KEY = "dsk-test";
    ...
    recordProviderFailure("gemini", new Error("429 rate limit"));
    assert.ok(getProviderHealth("gemini").cooldownUntil, "gemini should be cooling down");

    const seen: string[] = [];
    const m = mockFetch((url) => {
      if (url.includes("generativelanguage")) seen.push("gemini");
      else if (url.includes("openrouter")) seen.push("openrouter");
    ...
    // gemini is skipped (cooldown); live attempts continue through automatic providers.
    assert.ok(m.calls() <= 3 && m.calls() >= 1);
    assert.ok(!seen.includes("gemini"), "cooled-down gemini must not be called");
  });
});
...
describe("14. API keys and raw provider bodies are redacted", () => {
  it("never leaks the API key or raw body into the structured error", async () => {
    process.env.OPENAI_API_KEY = "sk-secret-openai-key-1234567890";
    // Body echoes a key-shaped secret; it must be redacted in any surfaced text.
    mockFetch(() => ({ status: 400, body: "error sk-secret-openai-key-1234567890 invalid max_tokens" }));
    try {
      await generateWithFallback("hi", { useCase: "proposal" });
      assert.fail("should have thrown");
    } catch (err) {
      const serialized = JSON.stringify((err as NoAiProviderReadyError).failureDetails) + (err as Error).message;
      assert.ok(!serialized.includes("sk-secret-openai-key-1234567890"), "API key must be redacted");
    }
    const health = getProviderHealth("zai");
    assert.ok(!(health.lastFailureMessage ?? "").includes("sk-secret-openai-key-1234567890"), "stored failure message must be redacted");
  });
});
...
describe("15. invalid JSON from automatic providers cannot promote requirements", () => {
  it("analyzeOneChunkWithRetry throws on malformed JSON (no requirements returned)", async () => {
    process.env.OPENAI_API_KEY = "sk-test";
    // OpenAI-compatible provider returns prose, not JSON. Validation must reject it — no requirements.
