...
describe("12. only three actual outbound attempts occur per request", () => {
  it("makes at most 3 outbound provider calls even when 5+ are configured", async () => {
    // Configure 5 automatic fetch-backed providers; all return HTTP 500 so the
    // chain keeps falling over. The budget must cap actual fetches at 3.
    process.env.OPENROUTER_API_KEY = "k";
    process.env.OPENROUTER_PROPOSAL_MODEL = "meta-llama/test:free";
...
describe("11. cooldown providers are skipped without consuming the budget", () => {
  it("skips a cooled-down provider and still tries 3 live providers", async () => {
    ...
    const m = mockFetch((url) => {
      ...
      else if (url.includes("anthropic")) seen.push("anthropic");
      return { status: 500, body: { error: { message: "fail" } } };
    });

    await assert.rejects(() => generateWithFallback("hi", { useCase: "proposal" }));
    // OpenRouter is skipped (cooldown); 3 live automatic providers are still attempted.
...
describe("15. invalid JSON from automatic OpenAI-compatible providers cannot promote requirements", () => {
  it("analyzeOneChunkWithRetry throws on malformed JSON (no requirements returned)", async () => {
    process.env.OPENAI_API_KEY = "k";
    // OpenAI returns prose, not JSON. Validation must reject it — no requirements.
