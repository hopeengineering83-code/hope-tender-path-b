...
describe("Submission-plan build route — zero GeneratedDocument.create", () => {
  ...
  it("build route uses 'virtual' status (not 'created')", () => {
    assert.ok(
      buildRoute.includes('status: "virtual"'),
      "Build route must use 'virtual' status for planned files",
    );
  });

  it("build route returns virtualOnly: true", () => {
    assert.ok(
      buildRoute.includes("virtualOnly: true"),
      "Build route response must include virtualOnly: true",
    );
  });

  it("build route sets created = 0 (no real rows created)", () => {
    assert.ok(
      buildRoute.includes("const created = 0"),
      "Build route must set created = 0",
    );
  });
});
