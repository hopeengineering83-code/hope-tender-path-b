...
describe("[SCREENSHOT-EXPORT-003] Item 1 — currency provenance UI", () => {
  it("report page renders 'Unverified legacy value' for corrupted tender with USD", () => {
    const src = readFileSync(resolve("app/dashboard/tenders/[id]/report/page.tsx"), "utf8");
    assert.ok(
      src.includes("Unverified legacy value"),
      "report page must render 'Unverified legacy value' for currency on corrupted tenders",
    );
    assert.ok(
      src.includes("CORRUPTED_STATUSES"),
      "report page must define a CORRUPTED_STATUSES set for the provenance check",
    );
    assert.ok(
      src.includes("analysisExtractionStatus"),
      "report page must select analysisExtractionStatus to determine currency provenance",
    );
  });
