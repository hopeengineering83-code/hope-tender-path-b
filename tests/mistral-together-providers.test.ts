...
describe("canonical provider chain", () => {
  ...
  it("derives the chain from the registry (gemini first, anthropic last)", () => {
    ...
    assert.ok(!chain.includes("together"), "Together must NOT be in the automatic canonical chain");
    assert.ok(!chain.includes("zai"), "Z.ai must NOT be in the automatic canonical chain");
  });
