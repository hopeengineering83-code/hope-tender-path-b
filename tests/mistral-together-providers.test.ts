...
describe("canonical provider chain", () => {
  ...
  it("derives the automatic chain from the registry (gemini first, anthropic last)", () => {
    ...
    assert.equal(chain[0], "gemini", "Gemini must be first in the canonical automatic chain");
    assert.equal(chain[chain.length - 1], "anthropic", "Anthropic/Claude must be last in the canonical automatic chain");
    assert.equal(chain.includes("mistral"), false, "Mistral must remain manual-only");
    assert.equal(chain.includes("together"), false, "Together must remain manual-only");
  });
