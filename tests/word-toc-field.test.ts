...
describe("final DOCX dynamic table of contents", () => {
  ...
  for (const tocHeading of ["#", "##"] as const) {
    it(`writes an updating Word TOC field for a ${tocHeading.length === 1 ? "level-1" : "level-2"} markdown TOC`, async () => {
      ...
      assert.equal(documentXml.match(/>Delivery Methodology<\/w:t>/g)?.length, 1);
      assert.ok(!documentXml.includes(">Nested static TOC entry</w:t>"));
      assert.match(settingsXml, /<w:updateFields(?:\s+w:val="true")?\s*\/>/);
    });
  }

  it("does not apply a Word heading style to the TOC title", async () => {
    const { documentXml } = await renderAndInspect("#");
    const titleIndex = documentXml.indexOf(">Table of Contents</w:t>");
    assert.ok(titleIndex > 0);
    const titleParagraph = documentXml.slice(
      documentXml.lastIndexOf("<w:p>", titleIndex),
      documentXml.indexOf("</w:p>", titleIndex),
    );
