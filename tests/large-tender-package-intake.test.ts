...
describe("large tender package intake", () => {
  ...
  it("accepts a bounded package larger than one request", () => {
    ...
    assert.deepEqual(partitionTenderUploadPackage(files).map((batch) => batch.length), [10, 1]);
  });
  ...
  it("rejects unsafe or excessive package selections before any request", () => {
    ...
    assert.match(validateTenderPackageSelection(Array.from({ length: 51 }, (_, index) => ({ name: `${index}.txt`, size: 1 }))) ?? "", /no more than 50 files/);
    assert.match(validateTenderPackageSelection([{ name: "oversized.pdf", size: MAX_TENDER_UPLOAD_FILE_BYTES + 1 }]) ?? "", /10 MB or smaller/);
    assert.match(validateTenderPackageSelection([{ name: "legacy.doc", size: 100 }]) ?? "", /unsupported format/);
    assert.match(validateTenderPackageSelection(Array.from({ length: 16 }, (_, index) => ({ name: `${index}.pdf`, size: 10 * 1024 * 1024 }))) ?? "", /exceeds 150 MB/);
  });
