...
describe("Route-level gate safety — PLANNED/SUPERSEDED never count as export-ready", () => {
  ...
  it("gate blocks when submissionPlanDocumentCount is 0 (no real documents)", () => {
    const input: GenerationReadinessInput = {
      ...
      requirementCount: 5,
      requirements: [
        { priority: "MANDATORY", sourceTenderFileId: "f1", sourcePageNumber: 1, sourceExactQuote: "meaningful quote text here", sourceFileActiveInTender: true },
      ],
      criticalMetadataOk: true,
      submissionPlanDocumentCount: 0, // No real documents — only PLANNED/virtual
    };
  ...
  it("gate allows when submissionPlanDocumentCount > 0 (real documents exist)", () => {
    const input: GenerationReadinessInput = {
      ...
      criticalMetadataOk: true,
      submissionPlanDocumentCount: 3, // Real documents exist
    };
    const result = evaluateGenerationReadiness(input);
...
describe("Route-level gate safety — USER_EDITED/USER_CONFIRMED only unblock with exact-match grounded evidence", () => {
  ...
  it("gate allows when criticalMetadataOk is true (USER_CONFIRMED with exact-match grounded evidence)", () => {
    ...
    const input: GenerationReadinessInput = {
      ...
      criticalMetadataOk: true, // USER_CONFIRMED with exact-match grounded evidence
      submissionPlanDocumentCount: 3,
    };
    const result = evaluateGenerationReadiness(input);
    assert.equal(result.ok, true);
