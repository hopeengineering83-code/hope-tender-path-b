...
function proseAnswer(prompt: string): string {
  ...
  // The section writer's contract asks for top-level ("# ") sections; content
  // above the first top-level heading does not reach the document.
  const topSection = (h: string, t: string) => `# ${h}\n\n${t}\n`;
  // The proposal's section prompts also name "company profile" — it is one of
  // this tender's requirements — so matching on that phrase alone answered
  // every proposal section with the company profile, and the proposal was
  // kept above the length gate only by a filler table (Section G) that no
  // longer ships. A section prompt carries the writer's "## YOUR OUTPUT" block.
  if (/company profile/i.test(prompt) && !/^## YOUR OUTPUT$/m.test(prompt)) {
    return [
...
before(async () => {
  ...
  // Route the app's REAL provider client at the local model.
  //
  // Groq is the provider used because it speaks the OpenAI wire format and its
  // base URL is overridable. It has no default model on purpose — the registry
  // requires an explicitly configured model for proposal, extraction and fast,
  // so isProviderConfigured("groq") stays false until all three are set. That
  // is the branch's "exact effective configured model" rule, not a defect, so
  // the test configures a model rather than relaxing the rule.
  process.env.GROQ_API_KEY = "local-model-key-not-a-real-secret";
  process.env.GROQ_BASE_URL = baseUrl;
  process.env.GROQ_PROPOSAL_MODEL = "local-acceptance-model";
  process.env.GROQ_ANALYSIS_MODEL = "local-acceptance-model";
  process.env.GROQ_FAST_MODEL = "local-acceptance-model";
  // Every other provider is cleared so the chain is deterministic and this
  // test never reaches for the network. Fallback ORDER is not what this test
  // covers; it covers whether the pipeline yields an artifact.
  for (const name of [
    "GEMINI_API_KEY", "MISTRAL_API_KEY", "ZAI_API_KEY", "CEREBRAS_API_KEY",
...
after(async () => {
  if (!RUN_DB) return;
  // PIPELINE_KEEP_FIXTURE=1 leaves the tender in place for inspection after a
  // failing run. Never set in CI; the cleanup below is the default.
  if (process.env.PIPELINE_KEEP_FIXTURE === "1") {
    console.log(`[pipeline-zip-test] keeping fixture: tenderId=${tenderId} userId=${userId}`);
