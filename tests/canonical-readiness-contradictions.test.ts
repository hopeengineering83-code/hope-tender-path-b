...
describe("canonical readiness contradictions restored coverage", () => {
  ...
  it("PLANNED documents are virtual/readiness-only in routes and are not persisted before readiness", async () => {
    ...
    assert.match(buildRoute, /virtual\/readiness-only/);
    ...
    const calls: unknown[] = [];
    const result = await hasValidSubmissionPlan({ generatedDocument: { count: async (args: unknown) => { calls.push(args); return 0; } } }, "tender-1");
