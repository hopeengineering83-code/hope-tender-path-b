...
content = re.sub(r'import \{.*\} from "./ai-provider-health";', import_replacement, content)
...
# 5. Remove top-level process.env reads (after replacing usages)
...
for p in ["anthropic", "openai", "mistral", "deepseek", "groq", "together", "openrouter"]:
    content = content.replace(f'recordProviderSuccess("{p}");', f'if (opts?.useCase === "extraction") {{ recordProviderAnalysisSuccess("{p}"); }} else {{ recordProviderSuccess("{p}"); }}')
