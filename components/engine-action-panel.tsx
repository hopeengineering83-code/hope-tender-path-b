...
export async function executeEngineRun(options: EngineRunOptions): Promise<void> {
  ...
  try {
    ...
    if (data.partial) {
      // The engine route sends blockers as string[] — extract from the
      // raw response. The EngineResponse type's `blockers` is
      // ExtractionBlocker[], so we read the raw field here.
