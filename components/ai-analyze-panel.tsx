...
export function AIAnalyzePanel({ tenderId, initialContinueJobId, aiEnabled }: Props) {
  ...
  const retryStatePollRef = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      if (retryStatePollRef.current) clearInterval(retryStatePollRef.current);
  ...
  function clearAllPolling() {
    ...
    if (retryStatePollRef.current) { clearInterval(retryStatePollRef.current); retryStatePollRef.current = null; }
  }
  ...
  // ── Retry state polling (for the current jobId) ───────────────────────
  async function fetchRetryState(jid: string) {
    try {
      // The retry state is embedded in the job output — we read it from
      // the job status poll. This is a lightweight separate poll that
      // runs only when the job has terminated with PARTIAL_SUCCESS/FAILED.
      ...
      if (output.retryCount !== undefined) {
        setRetryState({
          ...
          retryReason: output.retryReason ?? "",
          failureCategory: output.failureCategory ?? "",
          nonRetryable: Boolean(output.nonRetryable),
          lastProviderAvailable: Boolean(output.lastProviderAvailable),
        });
      }
    } catch { /* best-effort */ }
  }
  ...
  function scheduleAutoRetry(jid: string, retryCount: number) {
    cancelAutoRetry();
    if (retryCount >= MAX_RETRY_COUNT) {
      setAutoRetrySecondsLeft(null);
      return; // Limit reached — Retry Now remains available
    }
    ...
    autoRetryTimerRef.current = setTimeout(() => {
      ...
      // Only auto-retry if a provider is currently eligible
      if (providerAvail?.anyAvailable) {
        void handleAnalyzeBackground(jid);
      } else {
        // Provider not available — reschedule for the next delay slot
        // if we haven't exhausted retries
        if (retryCount + 1 < MAX_RETRY_COUNT) {
          scheduleAutoRetry(jid, retryCount + 1);
        }
      }
    }, delayMs);
  }
  ...
  async function handleAnalyzeBackground(existingJobId?: string) {
    ...
    try {
      // 1. Enqueue (if not resuming an existing job)
      if (!jid) {
        ...
        jid = enqueued.jobId;
        if (!jid) {
          setError("No job ID returned. Refresh the page.");
          setAnalyzing(false); setAnalyzeProgress(0); return;
        }
      }
      setJobId(jid);
  ...
  function pollJobStatus(jid: string) {
    ...
    async function checkOnce() {
      ...
      try {
        ...
        // Read retry state from output (recorded by run-next)
        const rsRetryCount = typeof output.retryCount === "number" ? output.retryCount : 0;
        const rsNonRetryable = Boolean(output.nonRetryable);
        const rsCategory = typeof output.failureCategory === "string" ? output.failureCategory : "";
        setRetryState({
          retryCount: rsRetryCount,
          nextRetryAt: typeof output.nextRetryAt === "string" ? output.nextRetryAt : null,
          retryReason: typeof output.retryReason === "string" ? output.retryReason : "",
          failureCategory: rsCategory,
          nonRetryable: rsNonRetryable,
          lastProviderAvailable: Boolean(output.lastProviderAvailable),
        });
  ...
  const showRetryUI = Boolean(error || (retryState && !retryState.nonRetryable));
  const autoRetryLimitReached = (retryState?.retryCount ?? 0) >= MAX_RETRY_COUNT;
        ...
        <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4">
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1">
              ...
              {retryState && !retryState.nonRetryable && !autoRetryLimitReached && autoRetrySecondsLeft !== null && (
                <p className="text-xs text-amber-800 mt-1">
                  Retry {retryState.retryCount + 1}/{MAX_RETRY_COUNT} • {retryState.retryReason}
                </p>
              )}
              {retryState?.nonRetryable && (
                <p className="text-xs text-red-800 mt-1">
                  Non-retryable: {retryState.retryReason}. Fix the root cause and click Retry Now.
                </p>
              )}
              {autoRetryLimitReached && !retryState?.nonRetryable && (
                <p className="text-xs text-amber-800 mt-1">
                  Automatic retry limit reached ({MAX_RETRY_COUNT} attempts). Click Retry Now to try again manually.
                </p>
              )}
              {providerAvail && providerAvail.configuredCount > 0 && providerAvail.anyAvailable && !autoRetryLimitReached && !retryState?.nonRetryable && (
                <p className="text-xs text-green-800 mt-1">
                  Provider recovered — resuming AI Analyze automatically.
                </p>
              )}
            </div>
