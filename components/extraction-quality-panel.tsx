...
import { EXTRACTION_STATUS_LABELS } from "../lib/engine/types";
...
type ExtractionQualityPanelProps = {
  tenderId: string;
  canonicalReadiness?: CanonicalTenderReadiness | null;
};

function formatPages(pages: number[], limit = 5): string {
  if (pages.length === 0) return "";
  const sorted = [...pages].sort((a, b) => a - b);
  if (sorted.length <= limit) return sorted.join(", ");
  return `${sorted.slice(0, limit).join(", ")} … (${sorted.length})`;
}

export function ExtractionQualityPanel({ tenderId, canonicalReadiness }: ExtractionQualityPanelProps) {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`/api/tenders/${tenderId}/extraction-quality`);
        const json = await res.json();
        if (!res.ok) throw new Error(json.error || "Failed to load extraction quality");
        setData(json);
      } catch (err) {
  ...
  if (loading) return <section className="mb-4 rounded-2xl border border-slate-200 bg-white p-5 text-sm text-slate-500 shadow-sm animate-pulse">Loading extraction quality…</section>;
  if (error || !data) return <section className="mb-4 rounded-2xl border border-red-200 bg-red-50 p-5 text-sm text-red-700 shadow-sm">Extraction quality error: {error}</section>;
  ...
  const hasWeakOrFailedPages = pageCoverage.failed > 0 || pageCoverage.weak > 0;
  const sectionCls = isCorruptionBlocked ? "border-red-200 bg-red-50" : ready ? "border-green-200 bg-green-50" : "border-amber-200 bg-amber-50";
  const headerTone = isCorruptionBlocked ? "text-red-700" : ready ? "text-green-700" : "text-amber-700";

  return (
    <section id="extraction-quality" className={`mb-4 rounded-2xl border p-5 shadow-sm ${sectionCls}`}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className={`text-xs font-semibold uppercase tracking-wide ${headerTone}`}>
            {canonicalReadiness?.modules.extraction && <CanonicalStatusIcon status={canonicalReadiness.modules.extraction.state} />} Extraction quality
          </p>
          <h2 className="mt-1 text-lg font-bold text-slate-900">{isCorruptionBlocked ? "Extraction blocked — quality too low" : ready ? "Page extraction appears usable" : "Extraction needs review"}</h2>
          <p className="mt-1 text-sm text-slate-600">Verifies text density, blank pages, OCR status, and whether critical tender sections (submission/evaluation/client) were detected across all files.</p>
        </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        <div className="rounded-xl bg-white p-3">
          ...
          <p className="text-xl font-bold text-slate-900">{pageCoverage.coveragePercent}%</p>
          <p className="text-[10px] text-slate-500">{pageCoverage.perfect}/{pageCoverage.total} perfect pages</p>
        </div>
        <div className="rounded-xl bg-white p-3">
          <p className="text-xs text-slate-500">Weak/Blank</p>
          <p className={`text-xl font-bold ${pageCoverage.weak > 0 ? "text-amber-600" : "text-slate-900"}`}>{pageCoverage.weak}</p>
        </div>
        <div className="rounded-xl bg-white p-3">
          <p className="text-xs text-slate-500">Failed</p>
          <p className={`text-xl font-bold ${pageCoverage.failed > 0 ? "text-red-600" : "text-slate-900"}`}>{pageCoverage.failed}</p>
        </div>
        <div className="rounded-xl bg-white p-3">
          <p className="text-xs text-slate-500">OCR Pages</p>
          <p className="text-xl font-bold text-slate-900">{pageCoverage.ocr}</p>
        </div>
        <div className="rounded-xl bg-white p-3">
      ...
      {!isCorruptionBlocked && (
        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-lg bg-white/50 p-2">
            <p className="text-[10px] font-semibold uppercase text-slate-500">Submission info</p>
          ...
          <div className="rounded-lg bg-white/50 p-2">
            <p className="text-[10px] font-semibold uppercase text-slate-500">Evaluation info</p>
          ...
          <div className="rounded-lg bg-white/50 p-2">
            <p className="text-[10px] font-semibold uppercase text-slate-500">Required docs</p>
          ...
          <div className="rounded-lg bg-white/50 p-2">
            <p className="text-[10px] font-semibold uppercase text-slate-500">Client details</p>
      ...
      {(analysisStatus === "PARTIAL_EXTRACTION_AI_ANALYZED" || analysisStatus === "EXTRACTION_WEAK_REVIEW_REQUIRED" || analysisStatus === "REGEX_FALLBACK_FROM_WEAK_EXTRACTION") && (
        <div className="mt-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          <strong>Warning: document is only partially extracted.</strong> AI Analyze ran on incomplete extraction ({EXTRACTION_STATUS_LABELS[analysisStatus as keyof typeof EXTRACTION_STATUS_LABELS] ?? analysisStatus}). Results may be missing tender pages. Re-extract or run OCR for a more reliable analysis.
        </div>
      )}
        ...
        {reports.map((item: any) => {
          ...
          return (
            <div key={item.id} className="rounded-xl border bg-white p-3 text-sm">
              ...
              {item.quality.warnings.length > 0 && <ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-slate-700">{item.quality.warnings.slice(0, 3).map((warning: string) => <li key={warning}>{warning}</li>)}</ul>}
            </div>
          );
        })}
      </div>
