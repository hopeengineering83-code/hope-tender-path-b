...
function aggregatePageStatus(reports: Array<{ fileName: string; perPage: PerPageExtractionReport }>) {
  ...
  for (const report of reports) {
    ...
    perfect += report.perPage.perfectPages.length;
  ...
  const coveragePercent = total > 0 ? Math.round((perfect / total) * 100) : 0;
  return { total, perfect, weak, failed, ocr, coveragePercent, lowConfidencePages, failedPages, submissionPages, evaluationPages, clientPages, requiredDocPages };
}
...
export async function ExtractionQualityPanel({ tenderId }: { tenderId: string }) {
  ...
  const reports = tender.files.map((file) => {
    ...
    return {
      ...
      usedStoredPageStatus: Boolean(stored && stored.length > 0),
    };
  });
    ...
    <section className={`mb-4 rounded-2xl border p-5 shadow-sm ${ready ? "border-green-200 bg-green-50" : "border-amber-200 bg-amber-50"}`}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">Page-status coverage: {pageCoverage.perfect}/{pageCoverage.total || "?"} good page(s) · {pageCoverage.coveragePercent}% coverage</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {!ready && (
            <a href="#legacy-tender-detail-actions" className="rounded-lg border border-amber-300 bg-white px-3 py-2 text-xs font-medium text-amber-800 hover:bg-amber-100">
              Re-extract / review ↓
            </a>
          )}
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-5">
        ...
        <div className="rounded-xl border bg-white p-3 text-sm"><p className="text-xs uppercase text-slate-500">Good pages</p><p className="mt-1 text-xl font-bold text-slate-900">{pageCoverage.perfect}</p></div>
        ...
        <div className="mt-4 rounded-xl border border-amber-200 bg-white p-4 text-sm">
          ...
          <div className="mt-3 grid gap-4 lg:grid-cols-3">
            <div>
              ...
              {pageCoverage.lowConfidencePages.length === 0 ? <p className="mt-1 text-xs text-slate-500">None reported.</p> : (
                <ul className="mt-1 list-disc space-y-1 pl-5 text-xs text-slate-700">
                  {pageCoverage.lowConfidencePages.slice(0, 10).map((page, index) => <li key={`${page.fileName}-${page.page}-${index}`}>{page.fileName} p.{page.page}: {page.reason}</li>)}
                </ul>
              )}
            </div>
            <div>
              <p className="text-xs font-semibold uppercase text-red-700">Blank / failed pages</p>
              {pageCoverage.failedPages.length === 0 ? <p className="mt-1 text-xs text-slate-500">None reported.</p> : (
                <ul className="mt-1 list-disc space-y-1 pl-5 text-xs text-slate-700">
                  {pageCoverage.failedPages.slice(0, 10).map((page, index) => <li key={`${page.fileName}-${page.page}-${index}`}>{page.fileName} p.{page.page}: {page.reason}</li>)}
                </ul>
              )}
            </div>
            <div>
              ...
              <ul className="mt-1 list-disc space-y-1 pl-5 text-xs text-slate-700">
                {pageCoverage.failed > 0 && <li>Re-extract the file or upload a cleaner source because blank/failed pages were detected.</li>}
                {pageCoverage.weak > 0 && <li>Review weak/table-heavy pages manually before relying on AI Analyze.</li>}
                {pageCoverage.coveragePercent < 80 && <li>Do not treat this as full extraction; page-status coverage is below 80%.</li>}
                {pageCoverage.coveragePercent >= 80 && pageCoverage.failed === 0 && <li>Continue only after reviewing weak/table-heavy pages.</li>}
              </ul>
            </div>
          </div>
        </div>
      ...
      {!ready && !isCorruptionBlocked && (
        <div className="mt-3 rounded-lg border border-amber-200 bg-white px-4 py-2.5 text-xs text-amber-900">
          <span className="font-semibold">Recommended action: </span>
          {anyOcrMissing
            ? "Set PDF_OCR_ENABLED=true and re-extract — the document requires OCR."
            : pageCoverage.failed > 0
              ? "Re-upload or re-extract the file. Blank/failed pages mean extraction is not fully reliable."
              : pageCoverage.coveragePercent < 80
                ? "Review or re-extract before AI Analyze. The page-status coverage is below the safe threshold."
                : "Continue only if the weak/table-heavy pages are acceptable for this tender."}
        </div>
      )}

      {isCorruptionBlocked && (
        <div className="mt-3 rounded-lg border border-red-300 bg-red-50 px-4 py-3 text-sm text-red-800">
          <strong>Extraction corrupted / OCR required</strong> — text quality is too low for reliable analysis. AI Analyze is blocked until the extraction quality issue is resolved.
        </div>
      )}
      ...
      {anyOcrRanForCorruption && (
        <div className="mt-3 rounded-lg border border-blue-300 bg-blue-50 px-4 py-3 text-sm text-blue-800">
          <strong>OCR was run to improve quality</strong> — the original text layer was detected as corrupted, so OCR was applied automatically.
        </div>
      )}

      {anyOcrMissing && (
        <div className="mt-3 rounded-lg border border-orange-300 bg-orange-50 px-4 py-3 text-sm text-orange-800">
          <strong>OCR required but not configured</strong> — the extracted text is corrupted and OCR would improve it, but OCR is currently disabled.
        </div>
      )}
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        {reports.map((item) => {
          ...
          const hasProblemPages = pp.blankPages.length > 0 || pp.failedPages.length > 0 || pp.lowDensityPages.length > 0 || pp.tableHeavyPages.length > 0;
          return (
            <div key={item.id} className="rounded-xl border bg-white p-3 text-sm">
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${item.quality.severity === "GOOD" && coveragePct >= 80 ? "bg-green-100 text-green-700" : item.quality.severity === "FAILED" || item.quality.severity === "POOR" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>
              ...
              {totalDetected > 0 ? (
                <div className="mt-2 space-y-1">
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <span>Page-status coverage</span>
                  ...
                  {item.totalPages && item.totalPages !== totalDetected && (
                    <p className="text-[11px] text-amber-700">Stored file page count is {item.totalPages}, but page-status review has {totalDetected} page row(s). Re-extract if this mismatch persists.</p>
                  )}
                  ...
                  {(pp.submissionInstructionPages.length > 0 || pp.evaluationCriteriaPages.length > 0 || pp.clientDetailPages.length > 0 || pp.requiredDocumentPages.length > 0) && (
                    <div className="mt-1.5 space-y-0.5 text-xs text-slate-500 border-t pt-1.5">
                      {pp.submissionInstructionPages.length > 0 && <div>Submission instructions: pp.{formatPages(pp.submissionInstructionPages, 5)}</div>}
                      {pp.evaluationCriteriaPages.length > 0 && <div>Evaluation criteria: pp.{formatPages(pp.evaluationCriteriaPages, 5)}</div>}
                      {pp.clientDetailPages.length > 0 && <div>Client/contact details: pp.{formatPages(pp.clientDetailPages, 5)}</div>}
                      {pp.requiredDocumentPages.length > 0 && <div>Required documents: pp.{formatPages(pp.requiredDocumentPages, 5)}</div>}
                    </div>
                  )}
                  {hasProblemPages && (
                    <div className="mt-1.5 border-t pt-1.5 space-y-0.5 text-xs">
                      {pp.failedPages.length > 0 && <p className="text-red-700">Failed pages: {formatPages(pp.failedPages, 10)}</p>}
                      {pp.blankPages.length > 0 && <p className="text-amber-700">Blank pages: {formatPages(pp.blankPages, 10)}</p>}
                      {pp.lowDensityPages.length > 0 && <p className="text-amber-700">Low-density pages: {formatPages(pp.lowDensityPages, 10)}</p>}
                    </div>
                  )}
                </div>
              ) : (
                <p className="mt-1 text-xs text-slate-500">No page-status rows are available. Re-run extraction or AI Analyze to rebuild page diagnostics.</p>
              )}

              <p className="mt-1 text-xs text-slate-400">
                {item.extractedCharacterCount.toLocaleString()} chars
                {item.quality.averageCharsPerPage ? ` · ~${item.quality.averageCharsPerPage}/page` : ""}
                {" · "}
