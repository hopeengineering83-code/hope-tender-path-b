    ...
    <section className={`mb-4 rounded-2xl border p-5 shadow-sm ${ready ? "border-green-200 bg-green-50" : "border-amber-200 bg-amber-50"}`}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {analysisStatus && (
            <span className={`rounded-full px-2 py-1 text-xs font-medium ${analysisStatus === "FULL_EXTRACTION_AI_ANALYZED" ? "bg-green-100 text-green-700" : analysisStatus === "PARTIAL_EXTRACTION_AI_ANALYZED" ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}`}>
              {EXTRACTION_STATUS_LABELS[analysisStatus] ?? analysisStatus}
            </span>
          )}
          {!ready && <a href="#legacy-tender-detail-actions" className="rounded-lg border border-amber-300 bg-white px-3 py-2 text-xs font-medium text-amber-800 hover:bg-amber-100">Re-extract / review ↓</a>}
          <Link href={`/api/tenders/${tenderId}/extraction-quality`} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50">Open JSON</Link>
        </div>
      </div>
        ...
        {reports.map((item) => {
          ...
          return (
            <div key={item.id} className="rounded-xl border bg-white p-3 text-sm">
              ...
              {totalDetected > 0 ? <div className="mt-2 space-y-1"><div className="flex items-center justify-between text-xs text-slate-500"><span>Page-status coverage</span><span className={`font-medium ${coveragePct >= 80 ? "text-green-700" : coveragePct >= 50 ? "text-amber-700" : "text-red-700"}`}>{goodCount}/{totalDetected} pages ({coveragePct}%)</span></div><div className="h-1.5 rounded-full bg-slate-100 overflow-hidden"><div className={`h-full rounded-full ${coveragePct >= 80 ? "bg-green-500" : coveragePct >= 50 ? "bg-amber-500" : "bg-red-500"}`} style={{ width: `${coveragePct}%` }} /></div>{item.totalPages && item.totalPages !== totalDetected && <p className="text-[11px] text-amber-700">Stored file page count is {item.totalPages}, but page-status review has {totalDetected} page row(s). Re-extract if this mismatch persists.</p>}<div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-xs text-slate-500 mt-1">{pp.ocrPages.length > 0 && <span>OCR pages: {pp.ocrPages.length}</span>}{pp.blankPages.length > 0 && <span className="text-amber-700">Blank: {pp.blankPages.length}</span>}{pp.lowDensityPages.length > 0 && <span className="text-amber-700">Low-density: {pp.lowDensityPages.length}</span>}{pp.failedPages.length > 0 && <span className="text-red-700">Failed: {pp.failedPages.length}</span>}{pp.tableHeavyPages.length > 0 && <span>Table-heavy: {pp.tableHeavyPages.length}</span>}</div>{(pp.submissionInstructionPages.length > 0 || pp.evaluationCriteriaPages.length > 0 || pp.clientDetailPages.length > 0 || pp.requiredDocumentPages.length > 0) && <div className="mt-1.5 space-y-0.5 text-xs text-slate-500 border-t pt-1.5">{pp.submissionInstructionPages.length > 0 && <div>Submission instructions: pp.{formatPages(pp.submissionInstructionPages, 5)}</div>}{pp.evaluationCriteriaPages.length > 0 && <div>Evaluation criteria: pp.{formatPages(pp.evaluationCriteriaPages, 5)}</div>}{pp.clientDetailPages.length > 0 && <div>Client/contact details: pp.{formatPages(pp.clientDetailPages, 5)}</div>}{pp.requiredDocumentPages.length > 0 && <div>Required documents: pp.{formatPages(pp.requiredDocumentPages, 5)}</div>}</div>}{hasProblemPages && <div className="mt-1.5 border-t pt-1.5 space-y-0.5 text-xs">{pp.failedPages.length > 0 && <p className="text-red-700">Failed pages: {formatPages(pp.failedPages, 10)}</p>}{pp.blankPages.length > 0 && <p className="text-amber-700">Blank pages: {formatPages(pp.blankPages, 10)}</p>}{pp.lowDensityPages.length > 0 && <p className="text-amber-700">Low-density pages: {formatPages(pp.lowDensityPages, 10)}</p>}</div>}</div> : <p className="mt-1 text-xs text-slate-500">No page-status rows are available. Re-run extraction or AI Analyze to rebuild page diagnostics.</p>}
