...
export function ExportReadinessPanel({ tenderId }: { tenderId: string }) {
  ...
  async function autoFinalize() {
    ...
    try {
      ...
      setRepairMessage(`Auto-finalize: processed ${data.processedCount ?? 0}, remaining ${data.remainingCount ?? 0}.`);
      await refresh();
    } catch (err) {
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {readiness && !ok && hasDocumentBlockers && (
            <button
              ...
              disabled={autoFinalizing || loading || repairing || linkingVault || supersedingOutsidePlan || Boolean(attachingDocId)}
              className="rounded-lg bg-blue-700 px-3 py-2 text-xs font-medium text-white hover:bg-blue-800 disabled:opacity-50"
            >
