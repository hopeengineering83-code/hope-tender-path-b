 ...
 *
 * Does NOT expose API keys, raw provider errors, or response bodies.
 */
  ...
  const perProvider = CANONICAL_AI_PROVIDER_ORDER.map((provider: AiProviderName, idx: number) => {
    ...
    if (!configured) {
      return {
        ...
        configured: false,
        coolingDown: false,
        cooldownUntilMs: null as number | null,
    ...
    return {
      ...
      status: snapshot.coolingDown ? ("COOLING_DOWN" as const) : ("AVAILABLE" as const),
    };
  });
