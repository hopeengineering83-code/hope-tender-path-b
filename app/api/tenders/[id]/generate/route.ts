...
import { childLogger, logger, reportError, time } from "../../../../../lib/observability";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (invalidFields.length > 0) {
    ...
    logger.warn(`[generate] tender=${tender.id} sanitised ${invalidFields.length} invalid stored field(s) before generation`, { route: "/api/tenders/[id]/generate", tenderId: tender.id, invalidFields });
  ...
  if (preGenValidation.warnings.length > 0) {
    logger.warn(`[generate] tender=${id} has metadata warnings`, { route: "/api/tenders/[id]/generate", tenderId: id, warnings: preGenValidation.warnings });
  }
  ...
  try {
    ...
    if (code === "P2021" || code === "P2010") {
      logger.warn("[generate] TenderMetadataOverride table not available — proceeding with empty overrides", { route: "/api/tenders/[id]/generate", tenderId: id, code, hint: "Run database migration to resolve." });
      metadataOverrides = [];
  ...
  if (untracedInitial.length > 0) {
    ...
    const repairResult = await repairSourceGrounding(id).catch((err) => {
      logger.warn("[generate] source-grounding repair attempt failed", { route: "/api/tenders/[id]/generate", tenderId: id, error: err instanceof Error ? err.message : String(err) });
      return null;
    });
