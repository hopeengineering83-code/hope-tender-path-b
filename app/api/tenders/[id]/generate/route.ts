...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (url.searchParams.get("planOnly") === "true") {
    ...
    const draftBuildPlan = await buildDraftBuildPlan(prisma, id, userId);
    if (!draftBuildPlan) {
      return NextResponse.json({ ok: false, code: "BUILD_PLAN_DRAFT_FAILED", error: "Unable to create draft Build Plan." }, { status: 422 });
    }
