...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!fallback && isSubstantial && (chunkNum === undefined || chunkNum === 3)) {
      ...
      const centralGate = await assertTenderReadyForGenerationAndExport({ prisma, tenderId: id, userId, purpose: "ai-proposal-persist" });
      if (!centralGate.ok) {
        await logAction({ userId, action: "AI_PROPOSAL_PERSIST_BLOCKED", entityType: "Tender", entityId: id, description: `Quick-draft persist blocked (${centralGate.blockerCode}): ${centralGate.blockerDetail}` }).catch(() => {});
      } else {
      try {
