...
async function validateGeneratedDocx(doc: {
  ...
  /*
    // Trust-level + draft markers (legacy)
    /AI_DRAFT/i,
    /REGEX_DRAFT/i,
    ...
    // AI / language-model self-references and provider names
    /\bas an AI\b/i,
    /\bAI-generated\b/i,
    /\blanguage model\b/i,
    /\bChatGPT\b/i,
    ...
    /\bprompt[\s:]+/i,
    // Internal traceability + scoring markers
    /\bsource snippet\b/i,
    /\bsource quote\b/i,
...
async function validateGeneratedFileForExport(doc: any) {
  const filename = (doc.exactFileName ?? generatedFileName(doc.name)).toLowerCase();
  if (filename.endsWith(".docx")) return validateGeneratedDocx(doc);
  if (filename.endsWith(".pdf")) return validateGeneratedPdf(doc);
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (type === "zip") {
    ...
    const finalZipEntries = zipScopeResult.entries.filter((entry) => {
      ...
      return Boolean(doc?.fileContent);
    });
