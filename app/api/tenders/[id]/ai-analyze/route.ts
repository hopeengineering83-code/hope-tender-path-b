      ...
      try {
        ...
        if (corruptedExtractionReports.length > 0) {
          await prisma.tender.update({
            ...
            logger.error("[ai-analyze/stream] failed to persist EXTRACTION_CORRUPTED_AI_SKIPPED status:", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
          });
        ...
        try {
          ...
          analysisJob = await prisma.$transaction(async (tx) => {
            ...
            if (existingRunning) {
              logger.warn(`[ai-analyze/stream] Concurrent AI job detected (${existingRunning.id}), reusing existing job instead of creating duplicate`, { route: "/api/tenders/[id]/ai-analyze", tenderId: id });
          ...
          if (jobCreateErr instanceof Error && jobCreateErr.message.includes("timeout")) {
            logger.warn("[ai-analyze/stream] Transaction timeout creating AI job — another request may be running analysis concurrently. Continuing with analysis...", { route: "/api/tenders/[id]/ai-analyze", tenderId: id });
          } else {
            logger.warn("[ai-analyze/stream] Failed to create AiJob record:", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: jobCreateErr instanceof Error ? jobCreateErr.message : String(jobCreateErr) });
          }
        }
        ...
        if (isAIEnabled()) {
          const onChunkStart = async ({ chunkIndex, totalChunks }: { chunkIndex: number; totalChunks: number }) => {
            ...
            await upsertAnalyzeChunkStarted({ tenderId: id, userId, contentHash, chunkIndex, totalChunks }).catch((e: unknown) => {
              logger.error("[ai-analyze/stream] checkpoint start write failed — chunk resume may retry this chunk:", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
            });
          };
          const onChunkComplete = async ({
            ...
            if (typeof chunkIndex === "number" && result) {
              ...
              await upsertAnalyzeChunkSucceeded({ tenderId: id, userId, contentHash, chunkIndex, totalChunks, result, provider }).catch((e: unknown) => {
                logger.error("[ai-analyze/stream] checkpoint succeeded write failed — chunk resume may retry this chunk:", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
              });
            }
            if (analysisJob) {
              await prisma.aiJob.update({
                ...
                logger.error("[ai-analyze/stream] AiJob partial output update failed (non-critical):", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
              });
            }
          };
          const onChunkFailure = async ({
            ...
            await upsertAnalyzeChunkFailed({ tenderId: id, userId, contentHash, chunkIndex, totalChunks, errorMessage, provider }).catch((e: unknown) => {
              logger.error("[ai-analyze/stream] checkpoint failed write failed — chunk may be retried on resume:", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
            });
          };
          try {
            ...
            void persistAllHealthToDb().catch((e: unknown) => {
              logger.error("[ai-analyze/stream] persistAllHealthToDb failed (non-critical):", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
            });
            ...
            await prisma.tender.update({ where: { id }, data: { analysisExtractionStatus: extractionStatus } }).catch((e: unknown) => {
              logger.error("[ai-analyze/stream] analysisExtractionStatus persist failed — generation gates may use stale status:", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
            });
            ...
            logger.error("[ai-analyze/stream] AI failed; regex fallback", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, category: diagnostics.category });
            void persistAllHealthToDb().catch((e: unknown) => {
              logger.error("[ai-analyze/stream] persistAllHealthToDb (fallback) failed (non-critical):", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
            });
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (isAIEnabled()) {
      try {
        ...
        try {
          ...
          logger.warn("[ai-analyze] Failed to create AiJob record — continuing without job tracking:", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: jobCreateErr instanceof Error ? jobCreateErr.message : String(jobCreateErr) });
        }
        ...
        const onChunkStartNonStream = async ({ chunkIndex, totalChunks }: { chunkIndex: number; totalChunks: number }) => {
          await upsertAnalyzeChunkStarted({ tenderId: id, userId, contentHash, chunkIndex, totalChunks }).catch((e: unknown) => {
            logger.error("[ai-analyze/non-stream] checkpoint start write failed — chunk resume may retry this chunk:", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
          });
        };
        const onChunkCompleteNonStream = async ({
          ...
          if (typeof chunkIndex === "number" && result) {
            await upsertAnalyzeChunkSucceeded({ tenderId: id, userId, contentHash, chunkIndex, totalChunks, result, provider }).catch((e: unknown) => {
              logger.error("[ai-analyze/non-stream] checkpoint succeeded write failed — chunk resume may retry this chunk:", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
            });
          }
          if (analysisJob) {
            await prisma.aiJob.update({
              ...
              logger.error("[ai-analyze/non-stream] AiJob partial output update failed (non-critical):", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
            });
          }
        };
        const onChunkFailureNonStream = async ({
          ...
          await upsertAnalyzeChunkFailed({ tenderId: id, userId, contentHash, chunkIndex, totalChunks, errorMessage, provider }).catch((e: unknown) => {
            logger.error("[ai-analyze/non-stream] checkpoint failed write failed — chunk may be retried on resume:", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
          });
        };
        let aiMeta: AnalysisWithMeta;
        try {
        ...
        void persistAllHealthToDb().catch((e: unknown) => {
          logger.error("[ai-analyze/non-stream] persistAllHealthToDb failed (non-critical):", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
        });
        ...
        await prisma.tender.update({
          ...
          logger.error("[ai-analyze/non-stream] analysisExtractionStatus persist failed — generation gates may use stale status:", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
        });
        ...
        const msg = aiError instanceof Error ? aiError.message : String(aiError);
        const diagnostics = buildAnalysisFallbackDiagnostics(msg);
        logger.error("AI analysis failed; deterministic fallback used", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, category: diagnostics.category, message: diagnostics.message });
        // Persist failure state so the next cold start knows which providers are cooling down.
        void persistAllHealthToDb().catch((e: unknown) => {
          logger.error("[ai-analyze/non-stream] persistAllHealthToDb (fallback) failed (non-critical):", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, error: e instanceof Error ? e.message : String(e) });
        });
    ...
    if (error instanceof AiAnalyzeCheckpointPersistenceError) {
      logger.error("Analysis route error (checkpoint persistence)", { route: "/api/tenders/[id]/ai-analyze", tenderId: id, code: error.code, diagnosticId: error.diagnosticId, operation: error.operation });
      return NextResponse.json(
        {
    ...
    void reportError(error, { route: "/api/tenders/[id]/ai-analyze", tenderId: id });
