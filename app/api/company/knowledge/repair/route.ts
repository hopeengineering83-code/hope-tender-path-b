...
async function buildDiagnostics(
  ...
  if (expertStates.length > 0 && reviewedExperts === 0) gaps.push({ severity: "MEDIUM", title: "Experts available for use", detail: `${sourceVerifiedExperts} source-verified and ${expertStates.length - sourceVerifiedExperts} draft/stale expert record(s) exist. Records are auto-approved and ready for use.` });
  if (projectStates.length > 0 && reviewedProjects === 0) gaps.push({ severity: "MEDIUM", title: "Projects available for use", detail: `${sourceVerifiedProjects} source-verified and ${projectStates.length - sourceVerifiedProjects} draft/stale project record(s) exist. Records are auto-approved and ready for use.` });
