...
type Asset = { id: string; assetType: string; originalFileName: string; mimeType: string; size: number; isActive: boolean; createdAt: string; storagePath?: string | null; hasInlineFileContent?: boolean | null; };
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {ASSET_TYPES.map((at) => {
            ...
            return <div key={at.value} className="rounded-2xl border bg-white p-5 shadow-sm">
              ...
              {active ? <div className="mt-3 rounded-lg bg-slate-50 p-3 text-xs space-y-2"><p className="font-medium text-slate-700 truncate">{active.originalFileName}</p><p className="text-slate-400">{fmt(active.size)} · {active.mimeType}</p>{active.hasInlineFileContent && !(active.storagePath ?? "").trim() && <p className="text-emerald-700 font-medium">Restored inline file available</p>}<div className="flex gap-2 pt-1"><button onClick={() => fileRefs.current[at.value]?.click()} disabled={isUploading} className="rounded bg-slate-200 px-2 py-1 text-xs text-slate-700 hover:bg-slate-300 disabled:opacity-60">Replace</button><button onClick={() => handleDelete(active.id)} disabled={deleting === active.id} className="rounded bg-red-100 px-2 py-1 text-xs text-red-700 hover:bg-red-200 disabled:opacity-60">Remove</button></div></div> : <button onClick={() => fileRefs.current[at.value]?.click()} disabled={isUploading} className="mt-3 w-full rounded-lg border-2 border-dashed border-slate-200 py-4 text-xs text-slate-400 hover:border-slate-400 hover:text-slate-600 disabled:opacity-60">{isUploading ? "Uploading…" : "+ Upload file"}</button>}
