...
type UploadProgress = { completed: number; total: number; phase: "creating" | "adding" };
type UploadFirstPayload = { error?: string; errors?: string[]; tenderId?: string };
...
export default function NewTenderPage() {
  ...
  async function handleUploadFirst() {
    ...
    try {
      ...
      const response = await fetch("/api/tenders/upload-first", { method: "POST", body: form });
      const data = await response.json().catch(() => ({})) as UploadFirstPayload;
      if (!response.ok) {
      ...
      const query = batches.length > 1
        ? `?${new URLSearchParams({ packageIntake: "1", packageFailed: String(failed) }).toString()}`
        : "";
      router.push(`/dashboard/tenders/${data.tenderId}${query}`);
    } catch {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const tender = await response.json() as { id?: string };
      if (!tender.id) {
    ...
    <div className="max-w-4xl space-y-6">
              ...
              <ul className="mt-3 space-y-2" aria-label="Selected tender documents">
                {files.slice(0, 12).map((file, index) => (
                  <li key={`${file.name}-${file.size}-${file.lastModified}`} className="flex min-w-0 items-center justify-between gap-3 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-600">
                    ...
                    <button type="button" onClick={() => removeTenderFile(index)} disabled={uploading} className="shrink-0 rounded-md border border-slate-300 bg-white px-2 py-1 font-medium text-slate-600 hover:bg-slate-100 disabled:opacity-60" aria-label={`Remove ${file.name}`}>Remove</button>
                  </li>
                ))}
              </ul>
              {files.length > 12 && <p className="mt-2 text-xs text-slate-500">And {files.length - 12} more selected document(s). All selected files remain part of the package.</p>}
            </>
          )}

          <div className="mt-4 flex flex-wrap gap-2">
            <button type="button" onClick={() => void handleUploadFirst()} disabled={uploading || files.length === 0} className="min-h-11 rounded-xl bg-blue-600 px-5 py-3 text-sm font-medium text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">{uploadButtonLabel}</button>
            {files.length > 0 && <button type="button" onClick={() => { setFiles([]); setUploadError(""); setUploadProgress(null); }} disabled={uploading} className="min-h-11 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-60">Clear selection</button>}
          </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border bg-white p-4 shadow-sm sm:p-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2"><label htmlFor="manual-tender-title" className="mb-1 block text-sm font-medium text-slate-700">Title *</label><input id="manual-tender-title" name="title" required placeholder="e.g. Urban master planning consultancy" className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black" /></div>
          <div><label htmlFor="manual-tender-reference" className="mb-1 block text-sm font-medium text-slate-700">Reference Number</label><input id="manual-tender-reference" name="reference" placeholder="e.g. RFP-2026-004" className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black" /></div>
          <div><label htmlFor="manual-tender-client" className="mb-1 block text-sm font-medium text-slate-700">Client</label><input id="manual-tender-client" name="clientName" placeholder="Client / procurement entity" className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black" /></div>
          <div><label htmlFor="manual-tender-category" className="mb-1 block text-sm font-medium text-slate-700">Category</label><select id="manual-tender-category" name="category" className="w-full rounded-lg border bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black">{CATEGORIES.map((category) => <option key={category}>{category}</option>)}</select></div>
          <div><label htmlFor="manual-tender-country" className="mb-1 block text-sm font-medium text-slate-700">Country</label><input id="manual-tender-country" name="country" placeholder="Country of procurement" className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black" /></div>
          <div><label htmlFor="manual-tender-method" className="mb-1 block text-sm font-medium text-slate-700">Submission Method</label><input id="manual-tender-method" name="submissionMethod" placeholder="Portal / email / hard copy" className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black" /></div>
          <div><label htmlFor="manual-tender-deadline" className="mb-1 block text-sm font-medium text-slate-700">Deadline</label><input id="manual-tender-deadline" name="deadline" type="date" className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black" /></div>
          <div><label htmlFor="manual-tender-budget" className="mb-1 block text-sm font-medium text-slate-700">Budget</label><input id="manual-tender-budget" name="budget" type="number" min="0" step="0.01" placeholder="0.00" className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black" /></div>
          <div><label htmlFor="manual-tender-currency" className="mb-1 block text-sm font-medium text-slate-700">Currency</label><select id="manual-tender-currency" name="currency" className="w-full rounded-lg border bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black">{CURRENCIES.map((currency) => <option key={currency}>{currency}</option>)}</select></div>
          <div className="sm:col-span-2"><label htmlFor="manual-tender-address" className="mb-1 block text-sm font-medium text-slate-700">Submission Address / Portal</label><input id="manual-tender-address" name="submissionAddress" placeholder="Portal URL, office address, or submission email" className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black" /></div>
          <div className="sm:col-span-2"><label htmlFor="manual-tender-description" className="mb-1 block text-sm font-medium text-slate-700">Description</label><textarea id="manual-tender-description" name="description" rows={3} placeholder="Brief description of the opportunity" className="w-full resize-none rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black" /></div>
          <div className="sm:col-span-2"><label htmlFor="manual-tender-summary" className="mb-1 block text-sm font-medium text-slate-700">Tender Intake Summary</label><textarea id="manual-tender-summary" name="intakeSummary" rows={6} placeholder="Paste known requirements, mandatory forms, expert needs, project experience rules, evaluation criteria, and file naming rules" className="w-full resize-none rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black" /></div>
          <div className="sm:col-span-2"><label htmlFor="manual-tender-notes" className="mb-1 block text-sm font-medium text-slate-700">Internal Notes</label><textarea id="manual-tender-notes" name="notes" rows={2} placeholder="Internal proposal notes (not included in submissions)" className="w-full resize-none rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black" /></div>
        </div>
        <div className="flex flex-wrap gap-3 pt-2">
          <button type="submit" disabled={loading} className="min-h-11 rounded-lg bg-black px-6 py-2 text-sm text-white hover:bg-slate-800 disabled:opacity-60">{loading ? "Creating…" : "Create Manual Tender"}</button>
