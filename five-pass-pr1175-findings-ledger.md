...
|---|---:|---|---|---|---|---|
| F001 | 3 | Critical | FIXED_LOCAL | Tender upload request paths called `extractTextFromBuffer` synchronously and directly queued analysis. They now persist verified bytes/source/package rows and exact hash-bound `EXTRACT_TEXT` jobs; the canonical worker owns extraction and continuation. | Preserve one transactionally owned source-to-job chain and fail closed on integrity/hash mismatch. | New contract failed 7/7 before the fix and passes 7/7 after; 382 focused/transitive assertions pass. Database/runtime proof remains open. |
...
| F013 | 3 | High | VERIFIED_LOCAL | `lib/ai-job-handlers-legacy.ts` retained another `EXTRACT_TEXT` implementation even though the registered handler delegated to `tender-extraction-service.ts`. The duplicate block is removed; the canonical service is the sole production owner. | Keep handler registration and tests bound to the canonical extraction service. | Handler-registration/dead-code assertions pass; repository search finds no legacy extraction implementation. |
| F014 | 3 | High | FIXED_LOCAL | Company-document upload persisted request-time extraction and queued `VAULT_INGEST` without guaranteeing background re-extraction. Upload now stores `PENDING` state and queues ingestion with `reExtractAll: true`; revision-zero queued documents advance through canonical extraction. | Keep Vault ingestion retryable and source-hash grounded; never promote request-time text. | Upload/ingestion source contracts and provenance consumers pass locally; isolated-database integration remains open. |
...
Latest result: TypeScript and ESLint clean; **382 tests passed, 0 failed**
across 92 suites. The earlier authority-only checkpoint was 126/126.

Database integration was not claimed: the workspace has no local PostgreSQL
service, and database-owning suites correctly refuse to run without
