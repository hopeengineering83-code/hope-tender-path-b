...
test.describe("settings API authorization, validation, ownership, and persistence", () => {
  test("GET exposes presentation semantics without tender facts", async ({ request }) => {
  ...
  test("invalid JSON, tender-fact fields, and cross-company targeting fail before writes", async ({ request }) => {
    ...
    const invalidJson = await request.fetch("/api/settings", {
      method: "PUT",
    ...
    for (const [field, value] of [
      ...
      const response = await request.put("/api/settings", { data: { [field]: value } });
    ...
    const invalidCurrency = await request.put("/api/settings", { data: { defaultCurrency: "NOT_A_CURRENCY" } });
  ...
  test("a valid own-company update is visible only after a server re-read and is restored", async ({ request }) => {
    ...
    try {
      const updateResponse = await request.put("/api/settings", { data: toggled });
      expect(updateResponse.status()).toBe(200);
      const updateBody = await readJson(updateResponse);
      expect(updateBody.changedFields).toContain("pageNumbering");

      const refreshedResponse = await request.get("/api/settings");
      ...
      const restoreResponse = await request.put("/api/settings", { data: original });
      expect(restoreResponse.status()).toBe(200);
      const restoredResponse = await request.get("/api/settings");
