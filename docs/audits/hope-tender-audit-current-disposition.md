...
| GAP-SEC-10 | Medium | `PARTIALLY_FIXED` | No SAST/DAST in CI workflow files. **However:** Hope has already enabled GitHub's **default CodeQL setup** on the repository (visible in CI as the `CodeQL` check run with `neutral` conclusion). Default setup runs CodeQL's `security-and-quality` query suite on every PR. An advanced-config workflow file (`.github/workflows/codeql.yml`) was initially added by this PR but **removed after CI showed it conflicts with the default setup** (error: "CodeQL analyses from advanced configurations cannot be processed when the default setup is enabled"). DAST (ZAP) is still not configured — defer to follow-up. **Operator action:** confirm default CodeQL setup covers all branches in repo settings → Code security → CodeQL analysis. |
