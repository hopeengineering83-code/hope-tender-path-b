...
| PR1175-F017 | HIGH | 4/5 | FIXED_LOCAL / RUNTIME OPEN | Raw FULL/SUBSTANTIAL rows could appear covered despite missing or invalid source trace, while the heatmap expanded one requirement into many compliance rows. One effective selector now requires active-file/page/exact-quote containment, emits `NEEDS_TRACE`, and drives the coverage API, all three panels, release snapshot and lifecycle controls. Exact-preview acceptance remains open. |
| PR1175-F018 | HIGH | 4/5 | FIXED_LOCAL / RUNTIME OPEN | Build Plan creation had three competing owners, page-load mutations, and unconfirmed scope presented as plan authority. One reconciliation owner now builds an explicit draft, shows every ordered item, requires acknowledgement, confirms through the strict route, and exposes `NOT_BUILT` / `DRAFT` / `CONFIRMED` / `STALE_CONFIRMED`; completeness is read-only and all row mutations are role-gated. Authenticated preview/database acceptance remains open. |
...
| PR1175-F021 | HIGH | 5 | FIXED_LOCAL / EXACT CI OPEN | Exact CI passed migrations, schema, drift, typecheck, lint, unit/database tests and build, then three authenticated Playwright tests asserted obsolete synchronous extraction immediately after upload (176 passed / 3 skipped / 3 failed). A shared acceptance helper now wakes the owned `EXTRACT_TEXT` worker and polls persisted source state before downstream assertions; upload expectations match `WAIT_FOR_SOURCE_EXTRACTION` / `EXTRACT_TEXT_QUEUED`. |
